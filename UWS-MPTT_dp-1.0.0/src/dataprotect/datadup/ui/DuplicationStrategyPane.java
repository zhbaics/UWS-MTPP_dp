...
import dataprotect.cmdp.ui.EditPhyProfileWhenInitDialog;
import dataprotect.cmdp.ui.PhyInitBootHostWizardDialog;
import dataprotect.cmdp.ui.multiRenderTable.JTableX3;
import dataprotect.cmdp.ui.multiRenderTable.MyDefaultTableModelForTabX3;
import java.awt.*;
...
import dataprotect.ui.SelectProtectedFS;
import dataprotect.ui.WizardDialogSample;
import java.io.File;
...
public class DuplicationStrategyPane extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        jPanel7.setLayout(new java.awt.FlowLayout(java.awt.FlowLayout.CENTER, 60, 5));
    ...
    private void myInit( SanBootView _view,int mode ){
        ...
        if( this.mode == ResourceCenter.CMD_TYPE_CMDP ){
            jTabbedPane1.remove(this.jPanel6);
        }
    }
    ...
    public void setIPAndPort( String ip,int port,int mtpp_port,String name,String machine,String os,String uuid,String bootMac,String aWinDir ){
        this.windir = aWinDir;
    ...
    public void setupTableForMTPP( ArrayList list ){
        ...
        for( int i=0; i<num; i++ ){
            ...
            data[i][0] = Boolean.valueOf(prof.isSel);
            data[i][1] = prof;
    ...
    public ArrayList getCMDPProfileItemFromTab(){
        PPProfile prof;
    ...
    public ArrayList getProfileInfo(){
        UniProfile prof;
        ...
        if( hidenFs_profile == null ){
            try {
                ...
                Logger.getLogger(DuplicationStrategyPane.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
    ...
    private UniProfile addHidenFsProfile() throws IOException {
...
SanBootView.log.error( getClass().getName(),"failed to create local tmp file when crt bakobj.");
               throw new IOException("can't crt tmp file when crt bakobj");
            }else{
...
SanBootView.log.error( getClass().getName(),"failed to send file to server when crt bakobj.");
                    tmpFile.delete();
                throw new IOException("can't send file to server when crt bakobj.");
                }
...
SanBootView.log.error( getClass().getName(),"failed to add bakobj.");
                throw new IOException("failed to add bakobj.");
                }
            }
        }
    if(hidenFsBkObj != null){
        UniProfile hidenFs_Profile = EditProfileWhenInitDialog.getHidenFsProfile(
        hidenFsBkObj,bkClnt,view.initor.user,view.initor.passwd,view.initor.getTxIP(),view.initor.port);
...
SanBootView.log.error( getClass().getName(),"failed to create local tmp file when gen hidenfs profile.");
            throw new IOException("can't crt tmp file when gen hidenfs profile.");
        } else {
...
SanBootView.log.error( getClass().getName(),"failed to send file to server when gen dup-hidenfs profile.");
            tmpFile1.delete();
            throw new IOException("can't send file to server when gen dup-hidenfs profile.");
            }
...
SanBootView.log.error( getClass().getName(),"failed to gen dup-hidenfs profile.");
            throw new IOException("failed to gen dup-hidenfs profile.");
            }else{hidenFs_Profile.isSel = true;}
        }
...
//        }
    ...
    private void addProfile(){
        EditProfileWhenInitDialog dialog = new EditProfileWhenInitDialog( view,host,bkClnt,new ArrayList<DBSchedule>(0),null,null,bootMac,selProtectFS,hidenFs );
    ...
    private void modCMDPProfile(){
        if( host == null ){
SanBootView.log.error( this.getClass().getName()," Can't find boothost in profile." );
...
//        if ( selectionline>=0 && selectionline <lineNum ){
            ...
            view.initor.mdb.getOSService( host.getIP(),host.getPort(),"",ResourceCenter.CMD_TYPE_CMDP );
            Vector serviceList = view.initor.mdb.getOSService();
            ((PhyInitBootHostWizardDialog)this.wizardDiag).addNewService( serviceList );

            ArrayList<PPProfileItem> curItemList = this.getCMDPProfileItemFromTab();
            ...
            this.setupTableForCMDP( this.oldProfList );
//        }
    }
...
SanBootView.log.warning( getClass().getName()," no d2d bkobj, bkobj id: "+ bkobjId );
            ...
            EditProfileWhenInitDialog dialog = new EditProfileWhenInitDialog( view, host, bkClnt, schList, bkObj,prof,bootMac,selProtectFS,this.hidenFs );
            if( host.isCMDPProtect() ){
                dialog.setFiltered_vol_list( this.getCMDPProfileItemFromTab() );
            }
            int width  = 560+ResourceCenter.GLOBAL_DELTA_WIDTH_SIZE;
