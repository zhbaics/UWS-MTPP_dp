...
public class PhyFailoverForSrcAgntWizardDialog extends WizardDialogSample implements FailoverInterface {
    ...
    private void myInit( SanBootView _view,SourceAgent _host,boolean _mustCheckDiskExist ){
        ...
        selectHostForNetBoot = new SelectHostForNetBootPane( view,null );
    ...
    @Override public void backButtonProcess(){
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("FailoverWizardDialog.title2.6").trim() ) == 0 ){
            ...
            if( selHostForNetBoot.mac.length() == 0 ){
                String rstIP1 = selHostForNetBoot.host.getIP();
                dhcpOnClnt.remove( rstIP1 );
                dhcpOnClnt.put( rstIP1, dhcpSetPane );
            }else{
...
SanBootView.log.info( getClass().getName()," @@#################  Assign drives End ###################@@ ");                
            ...
            for( i=0; i<size; i++ ){
                ...
                for( j=0; j<size1; j++ ){
                    ...
                    if( binder.getMDI().getSrc_agent_mp().toUpperCase().equals( part.disklabel.toUpperCase() ) ){
           ...
           // 将当前不需要的target的lunmap去掉
            ...
            for( i=0; i<size; i++ ){
                ...
                if( tid1 == -1 ){ // local disk
                    ...
                    isOk = view.initor.mdb.assignDriver( ip,port, 
                        view.initor.getTxIP(), ResourceCenter.ISCSI_LOGIN_PORT+"", part.iscsiVar,disk,ResourceCenter.CMD_TYPE_CMDP
                    );
                }
            ...
            if( useOdyDhcp ){
                ...
                if( isOk ){                                        
                    try {
                        ...
                        Logger.getLogger(CrtView1.class.getName()).log(Level.SEVERE, null, ex);
                    }
