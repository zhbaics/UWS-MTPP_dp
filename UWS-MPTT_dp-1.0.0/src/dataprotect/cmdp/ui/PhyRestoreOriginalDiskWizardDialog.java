...
import dataprotect.cmdp.service.GetBuildProgress;
...
import dataprotect.datadup.data.UniProIdentity;
...
public class PhyRestoreOriginalDiskWizardDialog extends WizardDialogSample implements RecordUWSRptable, RestoreOrigiDiskable, RestoreOriginalDiskInterface {
    ...
    public void nextButtonProcess() {
        ...
        if (getTipTitle().compareTo(SanBootView.res.getString("RestoreOriginalDiskWizardDialog.title3").trim()) == 0) {
            ...
            if (setRealstoreView()) {
                selectRestDiskPane.setJTextField1(driverletter);
            }
        ...
        if (getTipTitle().compareTo(SanBootView.res.getString("RestoreOriginalDiskWizardDialog.title").trim()) == 0) {
            if (partList == null) {
                ...
                if (volList.size() > 0) {
                    ...
                    //int rstmap_size = rstMapperList.size();
                    int rstmap_size = 0;
                    if (rstmap_size > 0) {
                        ret = JOptionPane.showConfirmDialog(
                                view,
            ...
            if (isFirst1) {
                if (pane instanceof ActModePaneForRestWinOrgDisk) {
                    ((ActModePaneForRestWinOrgDisk) pane).setManualRst(true);
                }
                isFirst1 = false;
            }
            refreshCenterPane();
    ...
    public boolean setRealstoreView() {
        boolean ret = true;
        String letter = "C";
        boolean isOk;
        View newView, bootView = null;
        BootVerList verlist = view.getBootVer(host.getID());
        if (verlist != null) {
            BindOfDiskLabelAndTid binder = verlist.osBootVer;
            if(binder != null){
                int local_snapid, root_id;
                view.initor.mdb.queryVSnapDB("select * from " + ResourceCenter.VSnap_DB + " where " + BasicVDisk.BVDisk_Snap_TID + "=" + binder.getTid());
                ArrayList list = view.initor.mdb.getAllQueryResult();
                if (list != null) {
                    for (int i = 0; i < list.size(); i++) {
                        BasicVDisk tmp = (BasicVDisk) list.get(i);
                        if (tmp.getTargetID() == binder.getTid()) {
                            bootView = new View(tmp);
                            break;
                        }
                    }
                } else {
                ...
                if (bootView != null) {
                    root_id = bootView.getSnap_root_id();
                    local_snapid = bootView.getSnap_parent();
                    view.initor.mdb.queryVSnapDB("select * from " + ResourceCenter.VSnap_DB + " where " + BasicVDisk.BVDisk_Snap_Root_ID + "=" + root_id);
                    Snapshot tmpshot = view.initor.mdb.getSnapshotFromQuerySql(root_id, local_snapid);
                    if (tmpshot != null) {
                        String newName = ResourceCenter.NET_START_VIEW + letter;
                        isOk = view.initor.mdb.addView(newName, tmpshot.getSnap_root_id(), tmpshot.getSnap_local_snapid());
                        if (isOk) {
                            newView = view.initor.mdb.getCrtView();
                            int tid = newView.getSnap_target_id();
                            String ip = host.getIP();
                            int port = host.getPort();
                            String iscsiVar = ResourceCenter.ISCSI_PREFIX + this.targetSrvName + ":" + tid;
                            isOk = view.initor.mdb.addLunMap(tid, ip, "255.255.255.255", "rw", "", "", "", "");
                            if (isOk) {
                                isOk = view.initor.mdb.getIdleDriveLetterForCMDP(ip, port);
                                if (isOk) {
                                    driverletter = view.initor.mdb.getIdleDrvLetter();
                                    if ( driverletter.length() != 0 ) {
                                        int retryCnt = 0;
                                        while (retryCnt < 3) {
                                            isOk = view.initor.mdb.assignDriver(ip, port, view.initor.getTxIP(),
                                                    ResourceCenter.ISCSI_LOGIN_PORT + "", iscsiVar, driverletter, ResourceCenter.CMD_TYPE_CMDP);
                                            if (isOk) {
                                                break;
                                            } else {
                                                ...
                                                try {
                                                    ...
                                                    Logger.getLogger(PhyRestoreOriginalDiskWizardDialog.class.getName()).log(Level.SEVERE, null, ex);
                                                }
                                                retryCnt++;
                                            }
                                        }
            ...
            if (row == num - 1) { // 保存信息( the last task )
                ...
                if (allFinished) {
                    ...
                    for (i = 0; i < size; i++) {
                        ...
                        view.initor.mdb.delAdjustDriverTmpFile(host.getIP(), host.getPort(), part.disklabel.substring(0, 1), ResourceCenter.CMD_TYPE_CMDP);
                    }
                ...
                if (allFinished) {
                    ...
                    for (i = 0; i < size; i++) {
                        ...
                        for (j = 0; j < size1; j++) {
                            ...
                            SanBootView.log.debug(getClass().getName(), " part info: " + part.disklabel);
                            if (part.disklabel.equals(dest.toUpperCase() + ":\\")) {
                                // 执行umount命令
                    ...
                    if (view.initor.mdb.getSystemPart(host.getIP(), host.getPort(), "ib_get_partition_info.exe", ResourceCenter.CMD_TYPE_CMDP)) {
                        ...
                        for (i = 0; i < size; i++) {
                            ...
                            for (j = 0; j < size1; j++) {
                                part = (SystemPartitionForWin) partList.elementAt(j);
                                SanBootView.log.debug(getClass().getName(), " part info: " + part.disklabel);
                                if (part.disklabel.substring(0, 1).toUpperCase().equals(dest.substring(0, 1).toUpperCase())) {
                                    errMsg = SanBootView.res.getString("RestoreOriginalDiskWizardDialog.log.umount") + dest + " "
                                            + SanBootView.res.getString("common.failed");
                                    runPane.setLogOnTabpane(errMsg, row);
                                    writeLogBuf(errMsg);
                ...
                if (profile == null) {
                    ...
                    if (tmpFile == null) {
                        ...
                        if (!view.initor.mdb.sendFileToServer(tmpFile.getName(), profile.prtMe())) {
                            ...
                            String bkClint_id = "";
                            if(bkClnt != null){
                                bkClint_id = Integer.toString((int) bkClnt.getID());
                            }
...
//                        registerOS = true;
                        ...
                        if (isRstOS(task.parm)) {
                            if (this.needModUUID) {
                                ...
                                isOkForHandleUUID = curUUID.length() != 0;
                                if (isOkForHandleUUID) {
                        ...
                        if (isOkForHandleUUID && isOkForCopy && isOkForStopService) {
                            ...
                            flushDisk(task.parm);
                            /*
                             * try{ SanBootView.log.info(getClass().getName(),
                             * "sleep 20 sec again after flush disk.....");
...
//                                        registerOS = false;
    ...
    public UniProfile collectRestInfo(Vector mpList, String snapBefCmd, String snapPostCmd, String mac) {
        ...
        String src;
        StringBuffer src_sb = new StringBuffer("");
        String target;
        ...
        if (!host.isWin2000()) {
            for (int i = 0; i < size; i++) {
                ...
                src_sb.append(binder.getRealSrc()).append(";");         //src += binder.getRealSrc() + ";";
                if (aIsFirst) {
                    target_sb = new StringBuffer(binder.getRealSrc() + "-?" + binder.dest);
                    aIsFirst = false;
                } else {
                    target_sb.append("-?").append(binder.getRealSrc()).append("-?").append(binder.dest);        //target += "-?" + binder.getRealSrc() + "-?" + binder.dest;
                }
            }
        } else {
            for (int i = 0; i < size; i++) {
                ...
                if (binder.src.toUpperCase().equals("C:")) {
                    if (mac.length() != 0) {
                        osDest = binder.dest;
                        src_sb = new StringBuffer(binder.getRealSrc());
                        target_sb = new StringBuffer(binder.getRealSrc() + "-?" + binder.dest);
                        break;
                    }
                } else {
        ...
        /*
         * //for wenjian's test on 2009-2-5 if( !host.isWin2000() ){
         * backup.setBakWithSnapFlag( "1" ); }else{ backup.setBakWithSnapFlag(
         * "0" ); }
        ...
        if (mac.length()!=0 && osDest.length()!=0) {
        ...
        iboot.setUws_ip(view.initor.getTxIP());
        ...
        while (list.hasMoreElements()) {
            ...
            if (aIsFirst) {
                if (!withLabel) {
                    ...
                    drivepath_sb = new StringBuffer(realSrc + "-?" + ResourceCenter.TARGET_TYPE + view.initor.getTxIP() + ":" + ResourceCenter.ISCSI_LOGIN_PORT + "@"
                            + ResourceCenter.ISCSI_PREFIX + this.targetSrvName + ":" + tid);
                }
                aIsFirst = false;
            } else {
                if (!withLabel) {
                    drivepath_sb.append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append( ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tid);
//                    drivepath += "-?" + ResourceCenter.TARGET_TYPE + view.initor.getTxIP() + ":" + ResourceCenter.ISCSI_LOGIN_PORT + "@"
//                            + ResourceCenter.ISCSI_PREFIX + this.targetSrvName + ":" + tid;
                } else {
                    drivepath_sb.append("-?").append(realSrc).append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tid);
//                    drivepath += "-?" + realSrc + "-?" + ResourceCenter.TARGET_TYPE + view.initor.getTxIP() + ":" + ResourceCenter.ISCSI_LOGIN_PORT + "@"
//                            + ResourceCenter.ISCSI_PREFIX + this.targetSrvName + ":" + tid;
                }
            }
        }

        String drivepath = drivepath_sb.toString();
        return drivepath;
    }
...
//    }
    ...
    private boolean stopOrStartServiceOnVg(Vector rstMapList, boolean isStart) {
        ...
        if (size > 0) {
            ...
            if (host.isOriginalBootHost() || host.isDstAgntUnderBootHost()) {
                ...
                PPProfile ppprof = view.initor.mdb.getBelongedDg(host.getSrc_Agnt_id(), srcDrive);
                if( ppprof != null){
                    PPProfileItem profileItem = ppprof.getMainDiskItem();
                    if( profileItem !=null ){
                        VolumeMap volMap = profileItem.getVolMap();
                        if( volMap != null ){
                                serv_list = volMap.getServices();
                        } else
                            serv_list = null;
                    } else
                        serv_list = null;
    ...
    private boolean checkDisk(Vector rstMapList) {
        ...
        for (i = 0; i < size; i++) {
            ...
            view.initor.mdb.checkDiskOnWin(ip, port, "1200", destdrive + ":", "0");
            // 不检查checkdisk的结果
            /*
             * if( !isOk ){ errMsg =
             * SanBootView.res.getString("RestoreOriginalDiskWizardDialog.error.checkDisk")
             * +" "+ destdrive; SanBootView.log.error(
             * getClass().getName(),"check disk failed on [ "+ ip +" / " +
             * port+" ] " +" for "+ destdrive ); return false; }
             */

            // 先umount,再mount一下本地目标盘
/*
             * // umount destdrive isOk = view.initor.mdb.assignDriver(
             * ip,port,"NULL","NULL",part.getVolInfo(),"0",ResourceCenter.CMD_TYPE_CMDP
             * ); try{ Thread.sleep( 10000 ); }catch(Exception ex){ } // mount
             * destdrive isOk = view.initor.mdb.assignDriver(
             * ip,port,"NULL","NULL",part.getVolInfo(),destdrive,ResourceCenter.CMD_TYPE_CMDP
            ...
            if (view.initor.mdb.getSystemPart(ip, port, "ib_get_partition_info.exe", ResourceCenter.CMD_TYPE_CMDP)) {
                ...
                for (j = 0; j < size1; j++) {
                    ...
                    if (newPart.disklabel.substring(0, 1).toUpperCase().equals(destdrive.substring(0, 1).toUpperCase())) {
                        founded = true;
    ...
    public RetValObj sizeIsMatched(String src, String dest) {
        ...
        for (i = 0; i < size; i++) {
            ...
            SanBootView.log.info(getClass().getName(), " partition: " + part.disklabel);
        ...
        if (srcPart != null && destPart != null) {
            ...
            SanBootView.log.info(getClass().getName(), "src disk total capacity: " + srcTotal + " [ " + src + " ]");
            if (srcTotal < 0) {
            ...
            SanBootView.log.info(getClass().getName(), "dest disk total capacity: " + destTotal + " [ " + dest + " ]");
            if (destTotal <= 0) {
                ret.errMsg = SanBootView.res.getString("RestoreOriginalDiskWizardDialog.log.badTotalSize");
                ret.isOk = false;
                return ret;
