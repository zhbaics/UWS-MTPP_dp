...
//                isOk = this.sendFileToAgent( conf );
                isOk = view.initor.mdb.updateDiskUUID(ip, port);
                if( isOk ){
                    String newdiskuuid = view.initor.mdb.getDiskUUID(new_diskno);
                    isOk = view.initor.mdb.restoreMBR(ip, port, "C", newdiskuuid);
                }
            }
            if( !isOk ){                
                errBuf.append(
