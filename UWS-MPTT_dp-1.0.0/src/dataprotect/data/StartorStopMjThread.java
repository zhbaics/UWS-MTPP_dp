...
import dataprotect.unlimitedIncMj.service.BasicStartorStopMjThread;
...
SanBootView.log.debug(getClass().getName(), "we will transfer a free volume.");
                    ...
                    BasicVDisk.TYPE_OPENED_AppMirr+") "+" and "+ BasicVDisk.BVDisk_Snap_Root_ID +"="+src_rootid+";"
                    );
                    if( ok ){
                ...
                if( ok ){
                    ...
                    if( !isOk ){
                        ...
                        UWSrvNode src_uws = view.initor.mdb.getUWSrvFromVecOnIP ( view.initor.getTxIP() ); 
                        if( src_uws == null ){
                            src_uws = new UWSrvNode( -1, view.initor.getTxIP(), UWSrvNode.MEDIA_SERVER_PORT, src_uws_psn );
...
SanBootView.log.info( getClass().getName(), "get src-agent according uuid: "+ uuid + " and uws server id: " + src_uws.getUws_id() );
                                ...
                                if( !isOk ){
                                    ...
                                    SourceAgent tmpSrcAgnt = view.initor.mdb.isThisUWSSrcAgntOnDestUWSrv1( view.initor.getTxIP() );
                                    if( tmpSrcAgnt == null ){
                                        // 代表源端UWS的srcagnt的uuid为空,src_agent_os字段表示psn
                                        isOk = crtSrcAgent( view.initor.getTxIP(), src_uws_psn, "","", src_uws.getUws_id(), -1,-1,0, 1 );
                                    }else{
                                        srcAgnt = tmpSrcAgnt;
                                    }
                                    if( isOk ){
...
System.out.println(" linux boot mac: " + linuxOrgBootMac );    
                        ...
                        if( dest_uws_tftproot.length() != 0 && src_uws_tftproot.length() != 0 ){
