...
import dataprotect.datadup.data.CronSchedule;
import dataprotect.res.ResourceCenter;
import dataprotect.ui.ChiefNode;
...
public class PPProfile extends ChiefNode implements TreeRevealable,TableRevealable{
    ...
    public boolean isValidDriveGrp(){
        return this.dg_name.length() != 0 && ( this.master_disk_rootid != -1 );
    }
    ...
    public PPProfileItem getMainDiskItem(){
        if( this.isValidDriveGrp() ){
            ...
            if( this.elements.size() > 0 ){
                PPProfileItem profileItem = this.elements.get( 0 );
                return profileItem;
            }else{
                return null;
            }
...
SanBootView.log.debug( getClass().getName(),"interval time: "+item.getMg().getMg_interval_time()+"" );
    ...
    public String getTempCronSchString(){
        ...
        if( this.isCronType() ){
            ...
            String clock_zone = this.temp_mg_schedule_clock_zone.length() == 0?"":", "+SanBootView.res.getString("common.clockZoneCtrl")+" "+this.temp_mg_schedule_clock_zone;
...
//                isFirst = false;
    ...
    public ServicesOnVolume convertToServicesOnVolume(){        
        ...
        PPProfileItem profileItem = this.getMainDiskItem();
        if( profileItem != null ){
            VolumeMap volMap = profileItem.getVolMap();
            if( volMap != null ){
                sv.setServiceList( volMap.getServices() );
            }
        }
        return sv;
    }
