...
import dataprotect.datadup.ui.ProcessEvent.ProcessEventOnChiefProfile;
import dataprotect.ui.InitNWinHostWizardDialog;
import dataprotect.ui.InitBootHostWizardDialog;
import java.awt.*;
...
private void jRadioButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jRadioButton2ActionPerformed
    ...
    private void setupLanguage(){
        if( this.bkobj == null ){
            setTitle(SanBootView.res.getString("EditProfileDialog.diagTitle"));
        }else{
            setTitle(SanBootView.res.getString("EditProfileDialog.diagTitle1"));
        }
...
//    }
    ...
    private void setupBakSourcePane( String bakList,String excludeList ){    
        ...
        jButton7.setEnabled( bakList == null || bakList.length() == 0 );
        jButton1.setEnabled( bakList != null && bakList.length() != 0 );
    }
...
//                hasEtcDir = true;
//            }
            filelist_sb.append(item).append(";");       //filelist += item + ";";
        }
    ...
    private String getExcludeList( boolean check ){
        ...
        size = model1.size(); 
        StringBuffer excludeListStr_sb = new StringBuffer("");
        for( i=0; i<size; i++ ){
            ...
            excludeListStr_sb.append(item1).append( ";");       //excludeListStr += item1 + ";";
        }
        String  excludeListStr =  excludeListStr_sb.toString();
        if( check ){
...
//            if( binder!=null ){
//                val = unixSrcList.get( binder.devno );
//                if( val == null ){
...
SanBootView.log.error( getClass().getName(), "\nLV is null in MDB: " + src +":\\" );
                ...
                mgInfo_sb.append(lv.getVolTargetID()).append("-?").append(lv.getMaxSnapNum()).append("-?").append(mgid).append(";");        //mgInfo += lv.getVolTargetID()+"-?"+lv.getMaxSnapNum()+"-?"+mgid+";";
            }
        }
...
SanBootView.log.debug(getClass().getName(), "====> " + binder.mp );
            ...
            if( bkobj == null || bkobj.getInclude().length() == 0 || bkobj.getInclude() == null ){
                bkdir = binder.mp;
            }else{
...
SanBootView.log.error(getClass().getName(), "\nVG or Target is null,exit." );
                   ...
                   if( isFirst ){
                        if( !withLabel ){
                            ...
                            drivepath_sb = new StringBuffer(bkdir+"-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
                                ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tgt.getVolTargetID());
                        }
                        isFirst = false;
                    }else{
                        if( !withLabel ){
                            drivepath_sb.append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tgt.getVolTargetID());
//                            drivepath+="-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                                ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tgt.getVolTargetID();
                        }else{
                            drivepath_sb.append("-?").append(bkdir).append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tgt.getVolTargetID());
...
SanBootView.log.error(getClass().getName(), "\nVG or Target is null,exit." );
                ...
                mgInfo_sb.append(tgt.getVolTargetID()).append("-?").append(tgt.getMaxSnapNum()).append("-?").append(";");   //mgInfo += tgt.getVolTargetID()+"-?"+tgt.getMaxSnapNum()+"-?"+mgid+";";
            }
        }
        String mgInfo = mgInfo_sb.toString();
        return mgInfo;
...
SanBootView.log.error( getClass().getName(), " data-duplication cmd errmsg: " + dup.getErrMsg() );
    ...
    public UniProfile collectBakInfo(){
        ...
        int intDelReData;        //int intDelReData = UniProBackup.DEFAULT_VAL_DEL_REDUNDANT;
        try{
        ...
        if(aBakObj!=null){backup.setExcludeFromBak( aBakObj.getExclude() );}
...
SanBootView.log.info( getClass().getName()," ### "+ bkClnt.getIP()+"'s dup content contains windir, must give iboot cmd.");
                ...
                Object[] val = InitBootHostWizardDialog.generateIboot_cmd( host.getBootMode(),host, tid, bootMac, view.initor.getTxIP() );
...
System.out.println(" windir(isOsDupForWin): "+ windir );        
    ...
    public static boolean realIsOsDupForWin( String file,String windir ){
        ...
        if( path.equals("/C:/") || path.equals("C:/") || path.equals("C:\\") ){
            return true;
        }
