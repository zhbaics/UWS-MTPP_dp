...
import dataprotect.datadup.cmd.*;
import dataprotect.datadup.data.*;
import dataprotect.datadup.ui.EditProfileDialog;
import dataprotect.datadup.ui.MonitorInfoReceiver;
import dataprotect.remotemirror.iBootInfo.GetIBootInfoFromTFtp;
import dataprotect.unlimitedIncMj.model.GetCloneDiskListCmd;
import dataprotect.unlimitedIncMj.model.table.CloneDisk;
import dataprotect.cmdp.entity.*;
import dataprotect.cmdp.service.*;
import dataprotect.remotemirror.cmd.GetMJScheduler;
import dataprotect.remotemirror.entity.MirrorJobSch;
...
public class ManageDB {
     ...
     * 扩展目标盘
     */
    NetworkRunningWithoutOutput expandTargetPartition;
    
    /*
     * 恢复目标盘的分区信息
     */
    NetworkRunningWithoutOutput restoreMBR;
    GetDiskUUID getDiskuuid;
    /*
     ** task 
     */
    ...
    */
    ...
    public ManageDB( SanBootView _view ){
        ...
        // volume 
        addVol = new AddOrphanVol( ResourceCenter.getCmd( ResourceCenter.CMD_ADD_VOL ));
        ...
        expandTargetPartition = new NetworkRunningWithoutOutput();
        restoreMBR = new NetworkRunningWithoutOutput();
        getDiskuuid = new GetDiskUUID();
    }
    ...
    public void closeStreamOnCurSocket(){
        try{
            ...
            expandTargetPartition.closeSocketStream();
            restoreMBR.closeSocketStream();
            getDiskuuid.closeSocketStream();
    ...
    public void setCurSocket( ){
        try{
            ...
            expandTargetPartition.setSocket( view.getSocket() );
            restoreMBR.setSocket( view.getSocket() );
            getDiskuuid.setSocket( view.getSocket() );
        }catch(Exception ex){
            ex.printStackTrace();
        }
    }
...
SanBootView.log.error( getClass().getName(), " mount IA hiden fs cmd errmsg: " + mountIAHidenFs.getErrMsg() );
    ...
    public String getHostName( String dest_uws_ip,int dest_uws_port,int pool_id,String pool_passwd ){
        if( targetSrvName == null || targetSrvName.length() == 0 ){
            if( dest_uws_ip.length() == 0 ){
...
SanBootView.log.error( getClass().getName(),"isStartupFromNetBoot cmd errmsg: "+isStartupFromNetBoot.getErrMsg() );              
    ...
    public boolean getIdleDriveLetterForCMDP( String cltIP ,int cltPort ){
        getIdleDriveLetter.setCmdLine(
                ResourceCenter.getCmdpS2A_CmdPath_FOR_LIST_CMD(cltIP, cltPort) +" drive -ava"
            );
        getIdleDriveLetter.setCmdType( ResourceCenter.CMD_TYPE_CMDP );
        try{
...
SanBootView.log.error( getClass().getName(),"loadInfo cmd errmsg: "+loadInfo.getErrMsg() );            
    ...
    // volInfo可以是一个形如<iqn>.iscsi.uws181:32768的字符串
    ...
    public boolean formatPartition( String ip,int port,String conf,int old_disk,int new_disk,String noFormat,int mode ){
        ...
        if( this.isMTPPCmd(mode)){
            ...
            formatPartition.setCmdLine(
                noFormat.length() == 0 ? cmdLine:cmdLine + " " +noFormat
            );
        }else{
...
SanBootView.log.error( getClass().getName(),"modify mirror_disk_info errmsg: " + modMdi.getErrMsg() );             
    ...
    public boolean addMDI( String dest_uws_ip,int dest_uws_port,int poolid,String passwd,MirrorDiskInfo mdi ){
     ...
     /*
            buf.append(" crsnap_services=" );
            buf.append(" changever_services=" );
            buf.append(" database_instances=" );
            buf.append(" others=" );          
            buf.append(" src_vol_capacity=0" );
            buf.append(" src_groupinfo=-1" );
            buf.append(" src_groupinfodetail=" );
            buf.append(" src_vol_info=-1" );
    ...
    public boolean isExistThisDisk( int rootid ){
        boolean ok = view.initor.mdb.queryVSnapDB(
            "select * from " + ResourceCenter.VSnap_DB +" where (" +BasicVDisk.BVDisk_Snap_OpenType +"="+
                    ...
                    BasicVDisk.TYPE_OPENED_AppMirr+") "+" and "+ BasicVDisk.BVDisk_Snap_Root_ID +"="+rootid+";"
        );
...
SanBootView.log.error(getClass().getName(),"addSnap cmd errmsg: "+addSnap.getErrMsg() );                   
    ...
    public boolean addSnapshotForCMDP( String clntIP,int port,String name,int isgrp, String desc,int mode ){
        ...
        if( !isOk ){
            ...
            }else if( addSnapForCmdp.getRet().equals("6")){
                this.errorMsg = SanBootView.res.getString("common.error.AsyncState");
            } else{
...
SanBootView.log.error( getClass().getName(),"Not found mg related rootid: " + rootid + " for host: " + host.getIP() );
    ...
    public PPProfile getBelongedDg( long clntID,String diskLabel ){
        ...
        for( int i=0; i<size; i++ ){
            PPProfile prof = profList.get( i );
            if( prof.isBelongedToThisDG( diskLabel ) ) return prof;
        }
        return null;
    }
...
/*
                if( item.getMg().getMg_pid() > 0 ){
                    if( !view.initor.mdb.activeTask( item.getMg().getMg_pid() ) ){
                        ok = false;
                    }
                }
*/
    ...
    public boolean updatePPProfileList(){
        ...
        for( i=0; i<size; i++ ){
            ...
            if( host.isMTPPProtect() ) continue;
...
SanBootView.log.error( getClass().getName(),"set workstate of sync cmd errmsg: "+setCurrentStateOfSync.getErrMsg() );
    ...
    public boolean getHAInfo( String clntIP,int port,String letter ){
        ...
        boolean isOk = !this.getHAInfo.isNull() || ( aErrCode == 0 ) || ( aErrCode == 1 ) || ( aErrCode == 255 );
...
SanBootView.log.error( getClass().getName(),"get HA info cmd errmsg: "+getHAInfo.getErrMsg() );
    ...
    public boolean isSupportHA(){
        ...
        // 所以只有为1时才认为“支持双机”，否则就是“不支持双机” （ 2010.11.15 ）
        return this.getHAInfo.getRetCode() == 1;
    }
...
SanBootView.log.error(getClass().getName(), "ib_driver_comp(symantec) cmd errmsg: "+ib_driver_comp.getErrMsg() );
     ...
     public boolean expandTargetPartition(String ip ,int port, String letter ){
        try{
            expandTargetPartition.setCmdLine(
                ResourceCenter.getCmdpS2A_CmdExpandPart(ip, port, letter)
            );
SanBootView.log.info( getClass().getName(),"expandTargetPartition cmd: "+expandTargetPartition.getCmdLine() );    
            expandTargetPartition.run();
        }catch(Exception ex){
            recordException( expandTargetPartition, ex );
        }
SanBootView.log.info( getClass().getName(),"expandTargetPartition cmd retcode: "+expandTargetPartition.getRetCode() );       
        boolean isOk = finished( expandTargetPartition );
        if( !isOk ){
SanBootView.log.error( getClass().getName(),"expandTargetPartition cmd errmsg: "+expandTargetPartition.getErrMsg() );                
        }
        return isOk;
    }
     
     public boolean restoreMBR (String ip ,int port ,String letter , String uuid) {
         try{
             restoreMBR.setCmdLine( ResourceCenter.getCmdpS2A_CmdRestoreMBR(ip, port, letter, uuid) );
             SanBootView.log.info( getClass().getName(), "restoreMBR cmd: " + restoreMBR.getCmdLine() );
             restoreMBR.run();
         }catch(Exception ex){
             recordException( restoreMBR , ex );
         }
         SanBootView.log.info( getClass().getName(), "restoreMBR cmd retcode: " + restoreMBR.getRetCode() );
         boolean isOk = finished( restoreMBR );
         if( !isOk ){
             SanBootView.log.info( getClass().getName() , "restoreMBR cmd errmsg: " + restoreMBR.getErrMsg() );
         }
         return isOk;
     }
     
     public boolean updateDiskUUID(String ip ,int port){
         try{
             getDiskuuid.setCmdLine( ResourceCenter.getCmdpS2A_CmdPath_FOR_LIST_CMD(ip, port) + " disk");
             SanBootView.log.info( getClass().getName(), "getDiskuuid cmd: " + getDiskuuid.getCmdLine() );
             getDiskuuid.clearCache();
             getDiskuuid.run();
         } catch(Exception ex){
             recordException( getDiskuuid , ex );
         }
         SanBootView.log.info( getClass().getName(), "getDiskuuid cmd retcode : " + getDiskuuid.getRetCode() );
         boolean isOk = finished( getDiskuuid );
         if( !isOk ){
             SanBootView.log.info( getClass().getName(), "getDiskuuid cmd errmsg : "+ getDiskuuid.getErrMsg() );
         }
         return isOk;
     }
     
     public String getDiskUUID( int diskno ){
         return getDiskuuid.getUUIDByDiskNo(diskno);
     }
}