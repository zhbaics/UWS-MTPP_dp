...
if(rst!=null){    SanBootView.log.info( getClass().getName(), " data-restore cmd: " + rst.getCmdLine() );}
                                rst.run();
                            }catch( Exception ex ){
                                if(rst != null){
...
SanBootView.log.error( getClass().getName(), " data-restore cmd errmsg: " + rst.getErrMsg() );
                                ...
                                while( !isFinished ){
                                    ...
                                    try {
                                        ...
                                        Logger.getLogger(RestoreOrigiDiskForUnixWizardDialog.class.getName()).log(Level.SEVERE, null, ex);
                                    }
                                }
...
SanBootView.log.info( getClass().getName()," ready to restore original disk using this ip: "+ host.getDst_agent_ip() );            
        ...
        for( int i=0; i<size; i++ ){
            ...
            src_sb.append(binder.src).append(";");           //src += binder.src+";";
            if( aIsFirst ){
                target_sb = new StringBuffer(binder.src+"-?"+binder.dest);
                aIsFirst = false;
            }else{
                target_sb.append("-?").append(binder.src).append("-?").append(binder.dest);        //target += "-?"+binder.src+"-?"+binder.dest;
            }
        }
        String target = target_sb.toString();
        String src = src_sb.toString();
        // 生成 identity 部分  ���
        ...
        boolean isSnapBefCmdNull = snapBefCmd.length() == 0;
        boolean isSnapAftCmdNull = snapPostCmd.length() == 0;
...
//                        ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tid;
                    ...
                    sb.append("-?").append(bkdir).append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tid);
//                    drivepath+="-?"+bkdir+"-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                        ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tid;
                }
