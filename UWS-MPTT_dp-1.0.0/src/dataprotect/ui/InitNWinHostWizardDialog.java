...
import dataprotect.datadup.ui.GetDataDupProcess;
...
public class InitNWinHostWizardDialog extends WizardDialogSample implements RecordUWSRptable,InitHostable {
    ...
    @Override public void backButtonProcess(){
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitNWinHostWizardDialog.title2").trim() ) == 0 ){
            ...
            if( oldInfo != null ) info.lvmTypeList = oldInfo.lvmTypeList;
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitNWinHostWizardDialog.title5").trim() ) == 0 ){
            ...
            ip = selectHostPane.getIP();
            profOnClnt.remove(ip);
            profOnClnt.put( ip,profInfo );
...
SanBootView.log.error( getClass().getName()," Modify d2d client failed: "+ip +"/" + port );                            
    ...
    @Override public void nextButtonProcess(){
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitNWinHostWizardDialog.title1").trim() ) == 0 ){
            ...
            if( getInitInfo.getGetAgentInfo() == null){
                    JOptionPane.showMessageDialog(view,
...
SanBootView.log.debug(getClass().getName(),"===========: "+binder.rawTgt.getCap() );
                ...
                if(lvmTypeList!=null){info.lvmTypeList = lvmTypeList;}
                partOnClnt.put(ip, info );
                if(lvmTypeList!=null){selectSysVolPane.setupTable( volList,lvmTypeList );}
...
//            port = selectHostPane.getPort();
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitNWinHostWizardDialog.title4").trim() ) == 0 ){ 
            ...
            contained = profOnClnt.containsKey(ip);
            if( !contained ){    
                if( aHost!=null ){
            ...
            if( aHost != null ){
                dupStrgyPane.setIPAndPort( ip,port,port,aHost.getName(), aHost.getMachine(),aHost.getOS(), aHost.getUUID(),mac,"" );
            }else{
                dupStrgyPane.setIPAndPort( ip,port,port,getAgentInfo.getHostName(), getAgentInfo.getMachine(), getAgentInfo.getOSName(), uuid,mac,"" );
            }
...
//System.out.println("updateGUI(updateOldHost)");
    ...
    private void initNewHostOnTree(){
        ...
        if( chiefHostNode != null )    chiefHostNode.add( hostNode );
    ...
    private boolean crtHost( String ip,int port ){
        ...
        if( host == null ){
            host = new BootHost(
                ...
                0,   // auto dr ?
                0,   // auto reboot?
                0,   // use default dhcp server(not 3rd part)?
                "",
                1,
                1
            );
...
SanBootView.log.error( getClass().getName(),"tgt name space is used up. It's impossible.");    
                    ...
                    tgtName = sb.append(tgtName).append("_").append(binder.part.mp).append(ip).toString();  //tgtName = tgtName + "_" + binder.part.mp+"_"+ip;
                    ...
                    if( isOk ){   
                        ...
                        if( isOk ){
                            ...
                            isOk = view.initor.mdb.addVg( ip,port, 
                                " -I " + iscsiVar +" -S " + view.initor.getTxIP() + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT +
                            ...
                            if( isOk ){
                                ...
                                if( isOk ){
                                    ...
                                    args = " -I "+iscsiVar +" -S "+ view.initor.getTxIP() + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT + " -t " + binder.targetID;
...
SanBootView.log.error(getClass().getName(),"target server name is null." );                        
                        ...
                        args = " -I "+iscsiVar +" -S "+ view.initor.getTxIP() + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT + " -t "+targetID;
                    ...
                    if( binder.isRealLV ){ // GUI上选中了已经存在的 lv
                        ...
                        if( view.initor.mdb.loginUnixTarget( ip,port,args )){
                            ...
                            if( isOk ){
                                ...
                                args = " -I "+iscsiVar +" -S "+ view.initor.getTxIP() + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT +" -lv "+binder.lvType;
                                StringBuffer sb = new StringBuffer(args);
                                sb.append(" -g ").append(binder.lv.getVolDesc());       //args += " -g "+binder.lv.getVolDesc();
                                sb.append(( binder.lvType.equals( GetLVMType.LVM_TYPE_NONE )?" -t "+targetID:" -l "+lvName ));          //args += ( binder.lvType.equals( GetLVMType.LVM_TYPE_NONE )?" -t "+targetID:" -l "+lvName );
                                sb.append(" -T ").append(binder.part.fsType);           //args += " -T "+binder.part.fsType;
                                args = sb.toString();
                                ...
                                if( binder.part.isEfiFlag() ){
                                    ...
                                    if( dev.length() != 0 && part_num.length() != 0 ){
                            ...
                            if( lv != null ){
                                ...
                                if( view.initor.mdb.loginUnixTarget( ip,port,args )){
                                    ...
                                    if( isOk ){
                                        ...
                                        args = " -I "+iscsiVar +" -S "+ view.initor.getTxIP() + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT +" -lv "+binder.lvType;
                                        StringBuffer sb = new StringBuffer(args);
                                        sb.append(" -g ").append(vg.getVolName());              //args += " -g "+vg.getVolName()
                                        sb.append( ( binder.lvType.equals( GetLVMType.LVM_TYPE_NONE )?" -t "+targetID:" -l "+lvName ) );     //args += ( binder.lvType.equals( GetLVMType.LVM_TYPE_NONE )?" -t "+targetID:" -l "+lvName );
                                        sb.append(" -T ").append(binder.part.fsType);   //args += " -T "+binder.part.fsType;
                                        args = sb.toString();
                                        isOk = view.initor.mdb.formatUnixVol( ip, port, args );
...
//        saveInfoStepOK = true;
        ...
        boolean isOkForSaveMp;
        boolean isOkForSavePartition;
        boolean isOkForSaveFstab;
        boolean isOkForSaveNetworkCmd;
        boolean isOkForSaveKillCmd;
        boolean isOkForSaveHaltCmd;
        boolean isOkForSaveBootInfo;
        boolean isOkForSaveOsLoaderType;
        boolean isOkForSaveDhcpInfo = false;
        boolean isOkForSaveLVMInfo;
        boolean isCopyOS;
...
SanBootView.log.debug(getClass().getName(), " old dhcp info is null "+" new mac: "+simpleMac );
            ...
            if( isOk ){                
                try {
                    ...
                    Logger.getLogger(InitNWinHostWizardDialog.class.getName()).log(Level.SEVERE, null, ex);
                }
...
SanBootView.log.error( getClass().getName()," Error: getting src_dest_mapper for profile failed.");
                sb = new StringBuffer(errMsg);
                sb.append(" : ").append(profName);
                errMsg = sb.toString();
                runningTaskPane.setLogOnTabpane( errMsg,RunningTaskPane.COPY_OS_ROW );
                writeLogBuf( errMsg, RunningTaskPane.COPY_OS_ROW );
                runningTaskPane.setTaskStatus(
...
SanBootView.log.error( getClass().getName()," Error: can't get os volume's target id.");
                ...
                Object[] val = InitNWinHostWizardDialog.generateIboot_cmd( tid, vg, bootMac, view.initor.getTxIP(), getProtectedMp( host.getID() ),
...
SanBootView.log.error(getClass().getName(), "\nVG or Target is null,exit." );
                ...
                StringBuffer mgInfo_sb =  new StringBuffer(mgInfo);
                mgInfo_sb.append(tgt.getVolTargetID()).append("-?").append(tgt.getMaxSnapNum()).append("-?").append(mgid).append(";");
                mgInfo = mgInfo_sb.toString();
            }
        }
...
System.out.println(" fs: "+fs );                      
                    if( src.startsWith( fs ) || (src+"/").startsWith( fs ) ){
...
//        String drivepath="";
        StringBuffer drivepath = new StringBuffer("");
...
SanBootView.log.debug(getClass().getName()," Check lunmap if exist: ip: "+host.getIP() +"  tid: "+tgt.getVolTargetID() );                    
                    ...
                    if( isFirst ){
                        if( !withLabel ){
                            drivepath.append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tgt.getVolTargetID());
...
//                                    ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tgt.getVolTargetID();
                            ...
                            drivepath.append(bkdir).append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tgt.getVolTargetID());
//                            drivepath = bkdir+"-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP() +":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                                ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tgt.getVolTargetID();
                        }
                        isFirst = false;
                    }else{
                        if( !withLabel ){
                            ...
                            drivepath.append(drivepath).append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tgt.getVolTargetID());
//                            drivepath+="-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP() +":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                                ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tgt.getVolTargetID();
                        }else{
                            drivepath.append(drivepath).append("-?").append(bkdir).append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tgt.getVolTargetID());
//                            drivepath+="-?"+bkdir+"-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP() +":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                                ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+tgt.getVolTargetID();
                        }
                   }
...
SanBootView.log.debug( getClass().getName(),"drivepath: "+drivepath.toString() );
...
SanBootView.log.error( getClass().getName(), "\nLV is null in MDB: target_id=" + tid );
            ...
            buf.append( "serverip-?");
            buf.append( view.initor.getTxIP() );
            buf.append( "-?tgtid-?" );
            buf.append( "" + tid );
            buf.append( "-?lvmtype-?" );
...
SanBootView.log.debug(getClass().getName(), " ele: "+list[i]);
                ...
                if( isFirst ){
                    path=new StringBuffer(list[i]);
                    isFirst = false;
                }else{
                    path.append(".").append(list[i]);
                }
            }
            
            if( path.length() == 0 ) path = new StringBuffer("root");
SanBootView.log.debug(getClass().getName()," path: "+ path.toString() );
            
            return "vol" + lastElement + "." + path.toString();
        }catch(Exception ex){
            return "";
        }
    }
    ...
    private boolean genIBootInfo( String iscsiVar,String ip,int port,int clntID,String extra ){
        ...
        String srv_ip = view.initor.getTxIP();
   ...
   */
    ...
    */
    ...
    private boolean crtLVorVgVolMap( String vgName,String lvName,String mp,int row,int clntId,String lvmType,String _snapSpace,int rootid ){
        ...
        if( lvName == null ){
            // 创建该vg对应的VolumeMap对象
            int snapSpace = Integer.parseInt( _snapSpace );
    ...
    private String getIBootConfContents( String iscsiVar,String clntIP,int clntID ){
        ...
        for( i=0; i<size; i++ ){
            ...
            if( isFirst ){
                tmp = new StringBuffer(tgt.getVolTargetID() + "");
                isFirst = false;
            }else{
                tmp.append(";").append(tgt.getVolTargetID());
            }
        }
        iBootConf.append("\ntargetlist="+tmp+"\n");
        ...
        for( i=0; i<size; i++ ){
            ...
            for( j=0; j<size1;j++){
                ...
                if( isFirst ){
                    ...
                    tmp.append(";").append(tgt.getVolTargetID());       //tmp+=";"+tgt.getVolTargetID();
                }
            }
            iBootConf.append("\npvs="+tmp);
            ...
            for( j=0; j<size1; j++ ){
                ...
                if( isFirst ){
                    tmp = new StringBuffer(lv.getVolName());        //tmp=lv.getVolName();
                    isFirst = false;
                }else{
...
//                        founded = true;
        ...
        for( i=0; i<size; i++ ){
            ...
            args = " -I "+iscsiVar +" -S "+ view.initor.getTxIP() + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT + " -t "+volMap.getVolTargetID();
...
System.out.println(" machine : " + machine );
        ...
        try {
            ...
            Logger.getLogger(GetInitInfoForUnixThread.class.getName()).log(Level.SEVERE, null, ex);
