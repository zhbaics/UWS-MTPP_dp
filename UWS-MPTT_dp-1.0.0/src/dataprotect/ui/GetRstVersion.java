...
import dataprotect.data.BasicVDisk;
import dataprotect.data.BindofVolAndSnap;
import dataprotect.data.BootHost;
import dataprotect.data.BriefVDisk;
import dataprotect.data.SnapWrapper;
import dataprotect.data.Snapshot;
import dataprotect.data.View;
import dataprotect.data.ViewWrapper;
import dataprotect.data.VolumeMap;
import dataprotect.data.VolumeMapWrapper;
import dataprotect.res.ResourceCenter;
import dataprotect.unlimitedIncMj.entity.CloneDiskWrapper;
...
public class GetRstVersion extends Thread{
    ...
    public GetRstVersion( ProgressDialog pdiag,SanBootView view,int hid,boolean isCMDP,boolean hasOS  ){
        this( pdiag,view,hid,isCMDP,hasOS,false );
    }

    public GetRstVersion( ProgressDialog pdiag,SanBootView view,int hid,boolean isCMDP,boolean hasOS,boolean isContainVolMap  ){
        this.pdiag = pdiag;
        this.view = view;
        this.hid = hid;
        this.isCMDP = isCMDP;
...
SanBootView.log.warning(getClass().getName(), "Can't get clone disk info on [ " + hid +"/0" +"/"+vol_rootid+" ]");
                ...
                for( j=0; j<size1; j++ ){
                    ...
                    if( isFirst ){
                        where_sb = new StringBuffer(BasicVDisk.BVDisk_Snap_Root_ID + "=" + cloneDisk.getRoot_id());
                        isFirst = false;
                    }else{
                        where_sb.append(" or ").append(BasicVDisk.BVDisk_Snap_Root_ID).append("=").append(cloneDisk.getRoot_id());        //where +=" or "+BasicVDisk.BVDisk_Snap_Root_ID + "=" + cloneDisk.getRoot_id();
                    }
                }
            }

            if( isFirst ){
                where_sb = new StringBuffer(BasicVDisk.BVDisk_Snap_Root_ID + "=" + vol_rootid);
                isFirst = false;
            }else{
                where_sb.append(" or ").append(BasicVDisk.BVDisk_Snap_Root_ID).append("=").append(vol_rootid);       //where +=" or "+BasicVDisk.BVDisk_Snap_Root_ID + "=" + vol_rootid;
            }
        }
        String where = where_sb.toString();
        ...
        //if( this.isCMDP ){
            ...
            while( bindList.size() > 0 ){
                ...
                if( volMap.isCMDPProtect() ){
                    PPProfile prof = view.initor.mdb.getBelongedDg( hid,volMap.getVolDiskLabel() );
