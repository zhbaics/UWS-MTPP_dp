...
SanBootView.log.debug( getClass().getName()," ##### changing drive letter: "+ mapper.getDest() +" ===> "+ mapper.getSrc() );
                ...
                for( j=0; j<size1; j++ ){
                    ...
                    if( dest.getMp().toUpperCase().equals( part.disklabel.toUpperCase().substring( 0,1 ) ) ){
...
SanBootView.log.warning( getClass().getName()," Volume info in partList is null for "+ dest.getMp() +", so we can not assign " + dest.getMp() +"-->" + mapper.getSrc() );
 ...
 SanBootView.log.debug(getClass().getName(), " #### Not restore disk: "+ part.disklabel );               
                try{
                    disk = part.disklabel.substring( 0,1 );
                    if( disk == null || disk.length() == 0 ) continue;
                }catch( Exception ex ){
                    continue;
                }
...
SanBootView.log.debug( getClass().getName(),"persisten vol(to be removed): "+ mapper.getSrc() );                  
                        ...
                        for( j=0; j<size1; j++ ){
                            part = (SystemPartitionForWin)partList.elementAt( j );
SanBootView.log.debug( getClass().getName(),"partition: "+ part.disklabel +" "+ part.volInfo );                      
                            if( mapper.getSrc().toUpperCase().equals( part.disklabel.toUpperCase().substring( 0,1 ) ) ){
         ...
         */
                    ...
                    if( !this.isSwitchDisk ){
                        ...
                        if( (tgtSrvName == null) || tgtSrvName.length() == 0 ){
