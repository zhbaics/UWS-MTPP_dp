...
public class PhyFailoverWizardDialog extends WizardDialogSample implements FailoverInterface{
    ...
    String selVer_info;   // save selected version info
    ...
    @Override public void backButtonProcess(){
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("FailoverWizardDialog.title2.5").trim() ) == 0 ){
            ...
            if( selHostForNetBoot.mac.length() == 0 ){
                String rstIP1 = selHostForNetBoot.host.getIP();
                cardOnClnt.remove( rstIP1 );
                cardOnClnt.put( rstIP1,netCardInfo );
            }
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("FailoverWizardDialog.title2.6").trim() ) == 0 ){
            ...
            if( selHostForNetBoot.mac.length() == 0 ){
                ...
                dhcpOnClnt.remove(rstIP2);
                dhcpOnClnt.put( rstIP2, dhcpSetPane );
            }else{
                dhcpOnClnt.remove( selHostForNetBoot.mac );
                dhcpOnClnt.put( selHostForNetBoot.mac, dhcpSetPane );
            }
    ...
    private ArrayList getNetCardList( ){
        ...
        boolean isOk;
        if(netBootedHost == null) return null;
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.selNetBootMode").trim() ) == 0 ){
            if( selectNetBootModePane.isSelectIScsiHBAMode() ){
                ...
                if( netBootedHost == null ){
                    ...
                    contained = cardOnClnt.containsKey( rstIP );
                    if( !contained ){
                        netCardList = getNetCardList(); 
                        cardOnClnt.put( rstIP, netCardList );
                        ...
                        Object val = cardOnClnt.get( rstIP );
...
//            }
            ...
            if( netBootedHost == null ){
                contained = dhcpOnClnt.containsKey( selHostForNetBoot.mac );
                if( !contained ){
...
SanBootView.log.debug( getClass().getName(),"selected netboot MAC: "+ mac ); 
                    ...
                    Object val = dhcpOnClnt.get( selHostForNetBoot.mac );
                ...
                contained = dhcpOnClnt.containsKey( rstIP ); 
                if( !contained ){
                    Object nc = selectNetCardPane.getSelectNetCard();
                    String mac="";
                    if( nc != null){
...
//                }catch(InterruptedException e){}
                ...
                if( getAgentInfo != null){
                    ...
                    errMsg = ResourceCenter.getCmdString( ResourceCenter.CMD_GET_HOST_INFO ) + " : "; 
                }
...
SanBootView.log.info( getClass().getName()," @@#################  Assign drives End ###################@@ ");                
            ...
            for( i=0; i<size; i++ ){
                ...
                for( j=0; j<size1; j++ ){
                    ...
                    if( binder.getVolMap().getVolDiskLabel().toUpperCase().equals( old_part.disklabel.toUpperCase() ) ){
...
SanBootView.log.info(getClass().getName(), "sleep 10 sec. and then retry set-partition again......");
                        try {
                            ...
                            Logger.getLogger(PhyFailoverWizardDialog.class.getName()).log(Level.SEVERE, null, ex);
                        }
                        retryCnt++;
                    }
                }
            ...
            if( !this.isSwitchDisk ){
                ...
                for( i=0; i<size; i++ ){
                    ...
                    tid1 = old_part.getTargetID();
                    disk = old_part.disklabel.substring( 0,1 );

                    if( tid1 == -1 ){ // local disk
                        if( old_part.disklabel.toUpperCase().equals("C:\\") ){
                            ...
                            if( selectHostForNetBoot.isKnownHost() ){
                                if( isOk ){
                                    runPane.setLogOnTabpane(
...
SanBootView.log.info( getClass().getName()," @@#################  Migration system from local disk to iSCSI End ###################@@ ");        
    ...
    private DestAgent getSelectedMC( ArrayList netCardList ){
        ...
        if( netCardList == null) return null;
        size = netCardList.size();
        for( i=0; i<size; i++ ){
                ...
                if( host.isEmBoot() ){
                    ...
                    if( isOk ){
                        if( view.initor.mdb.getFromVectorOnMac( simpleMac ) != null ){
                            ...
                            isOk = view.initor.mdb.addIboot( simpleMac, view.initor.getTxIP(), osTid );
                        }else{
                            isOk = view.initor.mdb.addIboot( simpleMac, view.initor.getTxIP(), osTid );
                        }
                        ...
                        if( isOk ){
                            isOk = view.initor.mdb.listIboot();
                            if( isOk ){
                                IBootObj ibootObj = view.initor.mdb.getFromVectorOnMac( simpleMac );
                                if( ibootObj != null ){
                                    if( !ibootObj.options.equals("000000a0") ){
                                        allFinish = false;
            ...
            if( useOdyDhcp ){
                ...
                if( isOk ){                 
                    try {
                        ...
                        Logger.getLogger(CrtView.class.getName()).log(Level.SEVERE, null, ex);
                    }
