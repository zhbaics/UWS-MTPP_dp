...
import dataprotect.datadup.data.BakTask;
...
public class GetDataDupProcess extends Thread {
    ...
    Runnable setVal = new Runnable(){
        public void run(){
            ...
            if( log.length() != 0  || prefix.length() != 0 ){
    ...
    private void parseTask( ){
        ...
        StringBuffer prefix_sb = new StringBuffer(prefix);
...
SanBootView.log.info( getClass().getName(), "finished task info: \n" + endTask.toString() );
                        prefix_sb.append(endTask);      //prefix += endTask.toString();
                    }
                }else{
