...
SanBootView.log.info( getClass().getName(),"delete adjust-driver-temp-file.");
                        ...
                        view.initor.mdb.delAdjustDriverTmpFile( host.getIP(),host.getPort(),part.disklabel.substring(0,1) );
                    }
...
SanBootView.log.error( getClass().getName(), " data-restore cmd errmsg: " + rst.getErrMsg() );
                                    ...
                                    while( !isFinished ){
                                        ...
                                        try {
                                            ...
                                            Logger.getLogger(RestoreOriginalDiskWizardDialog.class.getName()).log(Level.SEVERE, null, ex);
                                        }
                                    }
                ...
                for( i=0; i<size; i++ ){
                    ...
                    if( binder.src.toUpperCase().equals("C:") ){
                        ...
                        // 真正的命令参数
                        tmp_sb = new StringBuffer(tmp);
                        tmp_sb.append(" -c ").append(binder.getRealSrc()).append(" ").append(binder.dest);  //tmp += " -c " + binder.getRealSrc() + " " + binder.dest;
                        tmp = tmp_sb.toString();
                    }
                }
                
                copyOS = false;
                if( osDest.length() != 0 ){ // 需要拷贝操作系统ϵͳ
                    mac ="";
                    getMacOk = false;
                    
                    if( host.getDst_agent_mac().length() == 0 ){ // 表明源盘恢复是在初始化的原机器上进行的,所以要从保存在服务器上的配制文件中获取mac
...
SanBootView.log.error( getClass().getName(), " data-restore cmd errmsg: " + rst.getErrMsg() );
                ...
                if( profList.size() == 0 ){ // 某种原因，两个任务均不能提交运行
                    ...
                    String taskStr;
                    if( osDest.length() != 0 && tmp.length() != 0 ){
                        if( copyOS && copyData ){
...
SanBootView.log.error( getClass().getName(), "\nMirrorDisk is null in MDB: " + src );
            ...
            if( aIsFirst ){
                ...
                StringBuffer sb =  new StringBuffer(drivepath);
                if( !withLabel ){
                    sb.append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tid);
//                    drivepath+="-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                        ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":" + tid;
                }else{
                    sb.append("-?").append(realSrc).append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(tid);
//                    drivepath+="-?"+realSrc+"-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                        ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+ tid ;
                }
                drivepath =  sb.toString();
            }            
        }
...
SanBootView.log.info(getClass().getName(), " partition: "+part.disklabel );
            if( part.disklabel.equals( src.toUpperCase() ) ){
                srcPart = part;
            }else if( part.disklabel.equals( dest.toUpperCase() ) ){
                destPart = part;
            }
        }
