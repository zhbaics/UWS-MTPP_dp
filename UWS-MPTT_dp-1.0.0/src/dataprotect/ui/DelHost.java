...
public class DelHost extends BasicGetSomethingThread{
    ...
    public DelHost( SanBootView view,Object selHost,boolean isConnect ){
        super( view );
    ...
    public boolean realRun(){
        ...
        if( isWin() ) {
            ...
            // 1. 首先删除 service list
            Vector servList = view.initor.mdb.getAllServMapOnClntID( this.getHostID() );
            size = servList.size();
            for( i=0; i<size; i++ ){
            ...
            if( this.isMTPP() ){
                ...
                if( !delProfile() ){ return false;}
                if( !delPPProfile() ) { return false; }
            }
            
            // 3. 删除config
            if( !delConfigFile() ){ return false; }

            // 4. 删除启动配置( 目前不考虑删除dhcp和iboot的配置信息 )
            delIbootConfForWin();

            // 5. 删除 volume map
            Vector volList = view.initor.mdb.getVolMapOnClntID( this.getHostID() );
            size = volList.size();
            for( i=0; i<size; i++ ){
                volMap = (VolumeMap)volList.elementAt(i);
                
                // 如果可以连接上该client端，则删除client端上的mirrorring
                if( volMap.isCMDPProtect() && this.isConnect ){
                    if( !view.initor.mdb.delMirrorOnClnt( this.getHostIP(), this.getHostPort(), volMap.getVolDiskLabel().substring(0,1) ) ){
                ...
                // 删除disk上的lunmap
                if( !delLunMapOnVol( this.getHostIP(),this.getHostPort(),volMap ) ){ return false; }

                // 删除disk上所有快照的view的lunmap
                if( !delLunMapOnView( volMap ) ) { return false; }

                // 删除clonedisk及其view上的lunmap
                if( !this.delLunMapOnCloneDisk( volMap.getVol_rootid() ) ){ return false; }
            ...
            if( tgtSrvName.length() == 0 ) return false;
            ...
            if( isConnect ){
                for( i=0; i<size; i++ ){
                    ...
                    args = " -I "+iscsiVar +" -S "+ view.initor.getTxIP() + " -P "+ ResourceCenter.ISCSI_LOGIN_PORT + " -t "+tgt.getVolTargetID();        
                    ...
                    if( isOk ){
                        ...
                        if( !isOk ){
                            ...
                            // 删除clonedisk及其view上的lunmap
                            if( !delLunMapOnCloneDisk( tgt.getVol_rootid() ) ){ return false;}
...
SanBootView.log.info(getClass().getName(),"have active mjs on this mg,so can't stop this mg.");
    ...
    private boolean realDelHost( BrowserTreeNode chiefHostNode,BrowserTreeNode selHostNode ){
        Audit audit = view.audit.registerAuditRecord( this.getHostID(), MenuAndBtnCenterForMainUi.FUNC_DEL_HOST );

        boolean isOK = view.initor.mdb.delBootHost( this.getHostID() ); 
        if( isOK ){
    ...
    private boolean delPPProfile(){
        ...
        for( int i=0; i<size; i++ ){
            ...
            if( prof.isValidDriveGrp() ){
                ...
                if(master_disk == null) return false;
...
SanBootView.log.debug(getClass().getName(), "da ip: "+ da.getDst_agent_ip() );
    ...
    private boolean delLunMapOnCloneDisk( int rootid ){
        CloneDisk cd;

        boolean aIsOk = view.initor.mdb.getCloneDiskList( this.getHostID(),CloneDisk.IS_BOOTHOST,rootid );
        if( aIsOk ){
    ...
    private boolean delPersistentTargets( String ip,int port ){
        ...
        for( int i=0; i<size; i++ ){
            ...
            view.initor.mdb.delPersistentTarget( ip,port,persistent,ResourceCenter.CMD_TYPE_CMDP );
