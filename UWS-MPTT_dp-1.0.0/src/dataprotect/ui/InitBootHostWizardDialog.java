...
public class InitBootHostWizardDialog extends WizardDialogSample implements RecordUWSRptable,InitHostable {
    ...
    @Override public void backButtonProcess(){
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.title3").trim() ) == 0 ){
            ...
            servOnClnt.remove( ip );
            servOnClnt.put(ip,bindServ );
...
SanBootView.log.error(getClass().getName(), "failed to judge whether startup from localdisk.");
                ...
                for( i=0; i<size; i++ ){
                    ...
                    if( ahost != null ){
                        volMap = view.initor.mdb.generalGetVolMapFromVecOnClntandLabel( ahost.getID(), part.disklabel );
                        if( volMap !=null ){
...
SanBootView.log.debug( getClass().getName(),"===========: "+binder.vol.getCapStr1() );
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.title2").trim() ) == 0 ){
            ...
            contained = servOnClnt.containsKey( ip );
            if( !contained ){
...
//            port = selectHostPane.getMtppPort();
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.selNetBootMode").trim() ) == 0 ){
            if( selectNetBootModePane.isSelectIScsiHBAMode() ){
                ...
                if( ahost != null ){
                    dupStrgyPane.setIPAndPort( ip,port,port,ahost.getName(), ahost.getMachine(),ahost.getOS(), ahost.getUUID(),mac,aWinDir );
                }else{
                    dupStrgyPane.setIPAndPort( ip,port,port,getAgentInfo.getHostName(), getAgentInfo.getMachine(), getAgentInfo.getOSName(), uuid,mac,aWinDir );
                }
...
//            port = selectHostPane.getMtppPort();
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.title6").trim() ) == 0 ){ 
            ...
            if( !selectNetBootModePane.isSelectIScsiHBAMode() ){      
                NetCard card = (NetCard)selectNetCardPane.getSelectNetCard();
                if(card == null) return;
                runningTaskPane.ctrDhcpCb( setDhcpPane.isManualSetup(),card.mac );
            }else{
...
SanBootView.log.info( getClass().getName()," @@#################  Create Volume Begin ###################@@ ");        
        ...
        if( crtSize >0 ){
            ...
            for( i=0; i<crtSize; i++ ){
                ...
                if( binder.part.disklabel.substring(0,1).toUpperCase().equals("C") ){
                    // 表明OS volume 是创建出来的������
                    hasCrtOSDisk = true;
                }
                ...
                if( isOk ){
                    ...
                    if( binder.part.disklabel.substring(0,1).equals("C") ){
                        // 表明OS disk创建成功了
                        isCrtOSDiskOk = true;
                        OSBinder = binder;
                    }
...
SanBootView.log.error(getClass().getName(), " target server name is null.");                
                ...
                // add portal
                addPortal = view.initor.mdb.addPortal( ip, port, view.initor.getTxIP(),ResourceCenter.ISCSI_LOGIN_PORT );
                if( !addPortal ){
                    formatVolStepOK = false;
...
SanBootView.log.info( getClass().getName()," @@#################  Get other info Begin ###################@@ ");
        ...
        if( !selectNetBootModePane.isSelectIScsiHBAMode() ){
            ...
            if(netCard == null) netCard = new NetCard( "00-00-00-00-00-00" );
        }else{
...
SanBootView.log.error( getClass().getName()," Error: target server name is null.");                
                        ...
                        try {
                            ...
                            Logger.getLogger(InitBootHostWizardDialog.class.getName()).log(Level.SEVERE, null, ex);
                        }
                        ...
                        if( this.selectNetBootModePane.isSelectIBootMode() ){
                            ...
                            if( hasCrtOSDisk ){ // os volume 是创建产生的
                                while(OSBinder!=null){tid2 = OSBinder.targetID;}
                            }else{ // os volume使用已经存在的volume
                                Volume volume = selectSysVolPane.getSelectOSDiskVol();
                                if( volume !=null) tid2 = volume.getTargetID();
                            }
...
// 但这样做是否妥当，还需考虑(2008/3/30)
            ...
            if( needAddIboot && selectNetBootModePane.isSelectEmBootMode() ){
                if( hasCrtOSDisk ){ // os volume 是创建产生的
                    while(OSBinder!=null){tid1 = OSBinder.targetID;}
                }else{ // os volume使用已经存在的volume
                    Volume volume = selectSysVolPane.getSelectOSDiskVol();
                    if(volume != null) tid1 = volume.getTargetID();
                }
                DhcpClientInfo  olddhcpInfo = setDhcpPane.getOldDhcpClntInfo();
                String simpleMac;
                if( olddhcpInfo != null ){
SanBootView.log.info( getClass().getName(), " (iboot setup)find old dhcp info: "+olddhcpInfo.mac );
                    simpleMac = NetCard.getSimpleMac1( olddhcpInfo.mac );
SanBootView.log.warning( getClass().getName()," (iboot setup)old dhcp info is not null, new mac: " + simpleMac );
                }else{               
                    simpleMac = NetCard.getSimpleMac( netCard.mac );
...
SanBootView.log.error( getClass().getName(), "\nLV is null in MDB: " + src );
                ...
                if( isFirst ){
                    ...
                    StringBuffer sb1 = new StringBuffer(drivepath);
                    if( !withLabel ){
                        sb1.append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(lv.getVolTargetID());
//                        drivepath+="-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                            ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+lv.getVolTargetID();
                    }else{
                        sb1.append("-?").append(src).append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(view.initor.getTxIP()).append(":").append(lv.getVolTargetID());
//                        drivepath+="-?"+src+"-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                            ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+lv.getVolTargetID();
                    }
                    drivepath = sb1.toString();
                }
            }
        }
...
SanBootView.log.error( getClass().getName(), "\nLV is null in MDB: " + src +":\\" );
                ...
                StringBuffer sb = new StringBuffer(mgInfo);
                sb.append(lv.getVolTargetID()).append("-?").append(lv.getMaxSnapNum()).append("-?").append(mgid).append(";");       //mgInfo += lv.getVolTargetID()+"-?"+lv.getMaxSnapNum()+"-?"+mgid+";";
                mgInfo = sb.toString();
            }
        }
    ...
    private void sendNetbootInfoToDestUWS(){
        ...
        if( needSend ){
            if( hasCrtOSDisk&&OSBinder!=null){ // os volume 是创建产生的
                ...
                Volume volume = selectSysVolPane.getSelectOSDiskVol();
                if( volume !=null ) rootid = volume.getSnap_root_id();
            }
...
SanBootView.log.error( getClass().getName(),"Can't get disk partition info,so will not auto-restore-partition later. But we will tolerate this error."); 
        ...
        try {
            ...
            Logger.getLogger(GetInitInfoThread.class.getName()).log(Level.SEVERE, null, ex);
