...
import dataprotect.data.BindOfSnapAndView;
import dataprotect.data.BindofMdiAndSnap;
...
SanBootView.log.error( getClass().getName(),"creating new pxe config file in tftproot/pxelinux.cfg/<new_mac>  is failed. ");                        
                                ...
                                if( allFinish ){
                                    saveInfo( new_path + "/" + ((UnixFileObj)fList1.get(0)).getName() , newPxeInfo );
                                }   
                            }
            ...
            if( useOdyDhcp ){
                ...
                if( isOk ){
                    try {
                        ...
                        Logger.getLogger(CrtUnixViewForSrcAgent.class.getName()).log(Level.SEVERE, null, ex);
                    }
                    boolean hasdns = ( dhcpInfo.dns.length() != 0 );
                    boolean hasgw = ( dhcpInfo.defgw.length() != 0 );
...
//System.out.println( " mdi name: " + mdi.getSrc_snap_name() );
            ...
            if( isFirst ){
                tmp_sb = new StringBuffer(tid + "");
                isFirst = false;
            }else{
                tmp_sb.append(";").append(tid);     //tmp += ";" + tid;
            }
            replacePvs( ibootInfo, mdi.getSrc_snap_name(), tid );            
        }
        String tmp = tmp_sb.toString();
