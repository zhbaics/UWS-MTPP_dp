...
import mylib.tool.*;
import dataprotect.*;
import dataprotect.audit.ctrl.AuditCenter;
import dataprotect.cmdp.service.ProcessEventOnChiefPPProfile;
import dataprotect.cmdp.service.ProcessEventOnPPProfile;
...
import dataprotect.logging.*;
import dataprotect.remotemirror.ProcessEventOnChiefRollbackHost;
import dataprotect.remotemirror.service.ProcessEventOnChiefMjScheduler;
import dataprotect.remotemirror.service.ProcessEventOnMjScheduler;
...
import dataprotect.unlimitedIncMj.service.*;
...
public class SanBootView extends Browser{ 
    ...
    // 初始化文字资源
    public static Internationalization res = new Internationalization("dataprotect.res.SBBundle");
    ...
    /** Creates a new instance of MediaSrvView1 */
    ...
    public void myInit( int _mode ){
        ...
        InputStream is = null ;
        try{
            is = ResourceCenter.class.getResourceAsStream("buildtime");
            ...
            try {
                ...
                Logger.getLogger(SanBootView.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
        ...
        try{
            ...
            if(initor != null ){SanBootView.log.info( getClass().getName(), logBeginer + "  Log Level: " + initor.loglevel  );}
        }catch(Exception ex){
            ex.printStackTrace();
            JOptionPane.showMessageDialog(this,
                SanBootView.res.getString("common.error.initLog")
...
//System.out.println("current browserEvent type(getSelectedObjFromVssView): "+type );
    ...
    public BrowserTreeNode getProfNodeOnChiefProf( BrowserTreeNode chiefProfNode,String profName ){
        ...
        while( i<count ){
            ...
            name =(childNode.getUserObject()).toString();
    ...
    public BrowserTreeNode getHostNodeOnChiefHostNode( BrowserTreeNode chiefHostNode,int _cltID ){
        ...
        while( i<count ){
            ...
            cltID = ((BootHost)childNode.getUserObject()).getID();
            if( cltID == _cltID ){
                return childNode;
            }
...
SanBootView.log.debug(getClass().getName(), binder.prtMe() );         
    ...
    public Object[] getSomethingOnTreeFromChiefMjListObj( ChiefMirrorJobList chiefMj ){
        ...
        if( volObj instanceof Volume ){
            ...
            if( tgt != null ) root_id = tgt.getVol_rootid();
            type = "LogicalVol";
        }else if( volObj instanceof VolumeMap ){
...
SanBootView.log.warning(getClass().getName(),"create mj on unkonw type.");         
        ...
        ret[2] = type;
        ret[3] = Boolean.valueOf( isTransferBranch );
        return ret;
    }
    
    public Object[] getSomtthingOnTreeFromMjObj( MirrorJob mj ){   
        int rootid = 0,ptype = BootHost.PROTECT_TYPE_MTPP;
