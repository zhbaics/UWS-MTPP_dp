...
import dataprotect.ui.BasicGetSomethingThread;
import dataprotect.ui.GeneralActionForMainUi;
import dataprotect.ui.ProcessEventOnChiefHostVol;
    ...
    public boolean realRun(){
        ...
        if( this.isSyncOfVolMap && this.isMgStart ){
            ...
            if( this.isSyncOfVolMap && this.isMgStart ){
                ...
                if( !this.isSyncOfVolMap ){
                    errMsg = SanBootView.res.getString("MenuAndBtnCenter.error.startAutoCrtSnapFail")+":"+SanBootView.res.getString("MenuAndBtnCenter.error.startAutoCrtSnapFail1");
                } else {
                    errMsg = SanBootView.res.getString("MenuAndBtnCenter.error.startAutoCrtSnapFail")+":"+SanBootView.res.getString("MenuAndBtnCenter.error.startAutoCrtSnapFail2");
                }
                return false;
            }
        }
...
SanBootView.log.error( getClass().getName(), "can not get latest vol info for "+ volMap.getVolName() );
            ...
            if( newVolMap != null ){
                ...
                volMap.setVol_info( newVolMap.getVol_info() );
