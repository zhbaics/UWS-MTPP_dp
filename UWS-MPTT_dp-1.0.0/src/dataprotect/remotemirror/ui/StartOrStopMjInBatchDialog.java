...
import dataprotect.remotemirror.ui.multiRenderTable.JTableForMJ;
import dataprotect.remotemirror.ui.multiRenderTable.MyDefaultTableModelForMJ;
...
//                }catch(Exception ex){}
    ...
    public boolean init(){
        ...
        for( int i=0; i<size; i++ ){
            ...
            StringBuffer status_sb =  new StringBuffer(status);
            status_sb.append(" ").append(mj.getMj_job_name());      //status += " " + mj.getMj_job_name();
            status = status_sb.toString();
...
SanBootView.log.info(getClass().getName(), "this mirror job is a copy job: " + mj.getMj_id() );
        ...
        if( this.whatToDo == what_start ){
            return retValue && errormsg.length() == 0;
        }else{
            return retValue;
        }
    }
            ...
            if( hostObj instanceof BootHost ){
                ...
                if( _isOkTransferManagedInfo != null  ){
                    ...
                    if( val ){
                        ...
                        this.set_bootHost.put( new Integer(bh.getID()), isOkForTransfer );
                    }
                }else{
                    this.set_bootHost.put( new Integer(bh.getID()), isOkForTransfer );
                }
                ...
                if( _isOkTransferManagedInfo != null  ){
                    ...
                    if( val ){
                        ...
                        this.set_srcAgnt.put( new Integer( sa.getSrc_agnt_id() ), isOkForTransfer );
                    }
                }else{
                    this.set_srcAgnt.put( new Integer( sa.getSrc_agnt_id() ),isOkForTransfer );
                }
