...
public class ReCrtPPProfileWhenInit {
    ...
    PhyInitBootHostWizardDialog initDiag;
    ...
    private boolean containedInSet( String drive,Vector<BindOfPartandVol> disks ){
        ...
        for( int i=0; i<size; i++ ){
            ...
            if( !disk.isProtected ) continue;
            
            if( disk.part.disklabel.substring(0,1).toUpperCase().equals( drive.toUpperCase() ) ){
                return true;
            }
        }
        return false;
    ...
    private void adjustProfileDiskElement( PPProfile newProf ){
        ...
        for( int i=size-1; i>=0; i-- ){
            ...
            VolumeMap volMap = view.initor.mdb.getVolMapFromVecOnClntandLabel( host.getID(), item.getVolMap().getVolDiskLabel() );
            if( volMap == null ){
                elements.remove( i );
            }else{
...
SanBootView.log.info(getClass().getName(), "find volumes to delete, num# of del-vol is: " + delDiskSize );
        ...
        for( i=0; i<delDiskSize; i++ ){
            ...
            isOk = view.initor.mdb.destoryMirror( host.getIP(), host.getPort(), toDelVol.getVolName() );
            if( !isOk ){
                errMsg = SanBootView.res.getString("InitBootHostWizardDialog.log.delProtect")+" "+toDelVol.getVolDiskLabel() +" "+
                ...
                if( !isOk ){
                    ...
                    runPane.setLogOnTabpane(
                        SanBootView.res.getString("InitBootHostWizardDialog.log.delProtect")+" "+toDelVol.getVolDiskLabel() +" "+
        ...
        if( this.curNewPPPList == null) return false;
...
SanBootView.log.info(getClass().getName(), "find old profiles to delete, num of del-profile is: " + delOldSize );
        for( i=0; i<delOldSize ;i++ ){
            ...
            if( !oldProf.isValidDriveGrp() ) continue;
            if(oldProf.getMainDiskItem() == null) continue;
            PPProfileItem profileItem = oldProf.getMainDiskItem();
            if( profileItem != null ){
                volMap = profileItem.getVolMap();
            }
            if(volMap == null)continue;
            isOk = view.initor.mdb.delDG(
                oldProf.getDriveGrpName(),
                volMap.getVol_rootid()+"",
                volMap.getGroupinfodetail()
            );
            if( !isOk ){
...
SanBootView.log.info(getClass().getName(), "find new profiles to create, size of crt-profile is: " + crtNewSize );
        ...
        for( i=0; i<crtNewSize; i++ ){
            ...
            if( !newProf.isValidDriveGrp1() ) continue;
        ...
        if( crtNewSize > 0 ){
            ...
            for( i=0; i<crtNewSize; i++ ){
                ...
                PPProfile prof = view.initor.mdb.getPPProfile( host.getID(),PPProfile.getDiskList( newProf.getElements() ) );
                if( prof != null ){
