...
import dataprotect.datadup.ui.DuplicationStrategyPane;
import dataprotect.op.init.SelectNetBootModePane;
...
import dataprotect.cmdp.service.GetFreePhyVol1;
 ...
 */
public class PhyInitBootHostWizardDialog extends WizardDialogSample implements RecordUWSRptable,InitHostable {
    ...
    @Override public void backButtonProcess(){
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.selDisk").trim() ) == 0 ){
            ...
            oldInfo =(InfoFromProtectPane)partOnClnt.get( ip );
            info = new InfoFromProtectPane();
            if( oldInfo !=null) info.startNet = oldInfo.startNet;
            ...
            partOnClnt.remove( ip );
            partOnClnt.put( ip, info );
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.selNetBootMode").trim() ) == 0 ){
            ...
            ip = selectHostPane.getIP();
            bmodOnClnt.remove( ip );
            bmodOnClnt.put( ip, bootModeInfo );
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.title4").trim() ) == 0 ){
            ...
            ip = selectHostPane.getIP();
            cardOnClnt.remove( ip );
            cardOnClnt.put( ip, netCardInfo );
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.title5").trim() ) == 0 ){
            ...
            dhcpOnClnt.remove( ip );
            dhcpOnClnt.put( ip, dhcpSetPane );
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.title6").trim() ) == 0 ){
            ...
            ip = selectHostPane.getIP();
            ppprofOnClnt.remove( ip );
            ppprofOnClnt.put( ip, profInfo );
...
SanBootView.log.error( getClass().getName()," Modify d2d client failed: "+ip +"/" + mtpp_port );
    ...
    private ArrayList<UniProfile> crtPseudoLPProfile( String hostIP,int hostPort,BootHost host ){
        ...
        boolean contained = lpprofOnClnt.containsKey( hostIP );
        if( !contained ){
            if( host != null ){
            ...
            lpprofOnClnt.put( hostIP, profOfHost );
        }else{
            profOfHost = (ArrayList)lpprofOnClnt.get( hostIP );
        }
    ...
    private ArrayList<PPProfile> crtPseudoPPProfile( String hostIP,int hostPort,BootHost host ){
        ...
        // 获取GUI上一次设置的保护策略；如果是第一次，就从系统中找出对应的策略,没有则为空
        boolean contained = ppprofOnClnt.containsKey( hostIP );
        if( !contained ){
            if( host != null ){
            ...
            ppprofOnClnt.put( hostIP, profOfHost );
        }else{
            profOfHost = (ArrayList)ppprofOnClnt.get( hostIP );
        }        
        ...
        for( int i=0; i<size; i++ ){
            ...
            PPProfile prof = this.getBelongedDG( binder.part.disklabel, profOfHost );

            if( !binder.isProtected || binder.isProtectedByMTPP() ){
                if( prof != null ){
                    if( prof.isMasterDisk( binder.part.disklabel ) ){
                        prof.setMasterDiskRootid( -1 );
                    }
                    prof.removeItem( binder.part.disklabel );
                ...
                if( prof == null ){
                    ...
                    volMap = new VolumeMap( binder.volName,binder.part.disklabel );
                    mg = new MirrorGrp(
                    ...
                    if( prof.getMainDiskLabel().toUpperCase().equals( binder.part.disklabel.toUpperCase() ) ){                   
    ...
    @Override public void nextButtonProcess(){
        ...
        Vector partList = null,volList,subnetList,ibootSrvList;
...
SanBootView.log.error( getClass().getName()," ##### host is not windows. uuid: "+ uuid +" ip: "+ ip );                
                ...
                if( getAgentInfo == null ){
                    JOptionPane.showMessageDialog( view,
                        SanBootView.res.getString("MenuAndBtnCenter.error.gettingAgentInfo")
                    );
                    return;
                }
                // 检查平台是否冲突
...
SanBootView.log.debug( getClass().getName(),"  volmap object is in unsel hash table: "+ tid +" pick it into selected hash table." );
            ...
            contained = partOnClnt.containsKey( ip );
            if( !contained ){
                ...
                if( getInitInfo.getSysPartList() != null ){
                ...
                for( i=0; i<size; i++ ){
                    ...
                    if( ahost != null ){
                        volMap = view.initor.mdb.getVolMapFromVecOnClntandLabel( ahost.getID(), part.disklabel );
                        if( volMap != null ){
...
SanBootView.log.debug( getClass().getName(),"===========: "+binder.vol.getCapStr1() );
                                ...
                                if( ahost.isMTPPProtect() ){
                                    ...
                                    if( prof != null ){
                                        ...
                                        if(master_disk != null) binder.maxSnap = master_disk.getMg().getMg_max_snapshot()+"";
                                    }else{
                                ...
                                binder.volName=getDefaultVolName( part.disklabel, ip );
                            }
                        }else{
                            binder.volName =getDefaultVolName( part.disklabel,ip );
                        }
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.selDisk").trim() ) == 0 ){
            ...
            contained = bmodOnClnt.containsKey( ip );
            if( !contained ){
                if( ahost != null ){
                    if( ahost.isEmBoot() ){
                        bmodOnClnt.put( ip, new Object[]{ new Integer(BootHost.BOOT_MODE_EM ) } );                               
                        selectNetBootModePane.setEmBootMode( true );
                        selectNetBootModePane.clickJRadioBtn1();
                    }else if( ahost.isIBoot() ){
                        bmodOnClnt.put( ip, new Object[]{ new Integer(BootHost.BOOT_MODE_IBOOT ) } );                               
                        ...
                        bmodOnClnt.put( ip, new Object[]{ new Integer(BootHost.BOOT_MODE_ISCSI_HBA ) } );
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.selNetBootMode").trim() ) == 0 ){
            if( selectNetBootModePane.isSelectIScsiHBAMode() ){
                ...
                if( ahost != null ){
                    dupStrgyPane.setIPAndPort( ip,port,mtpp_port,ahost.getName(), ahost.getMachine(),ahost.getOS(), ahost.getUUID(),"","" );
                }else{
                    dupStrgyPane.setIPAndPort( ip,port,mtpp_port,getAgentInfo.getHostName(), getAgentInfo.getMachine(), getAgentInfo.getOSName(), uuid,"","" );
                }
                ...
                contained = cardOnClnt.containsKey( ip );
                if( !contained ){
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.title4").trim() ) == 0 ){
            ...
            contained = dhcpOnClnt.containsKey( ip );
            if( !contained ){
                ...
                if( ahost != null ){
                    ...
                    if( ip_3rd.length() == 0 )
                        ip_3rd = ip;
                    if( nextsrv_3rd.length() == 0 )
                        nextsrv_3rd= view.initor.getTxIP();
                }else{
                    ip_3rd = ip;
                    nextsrv_3rd= view.initor.getTxIP();
                }
                ...
                setTipTextOnTopPane( SanBootView.res.getString("InitBootHostWizardDialog.title5") );  
                NetCard card = new NetCard( "00-00-00-00-00-00" );
                if( selectNetCardPane.getSelectNetCard() != null){
                    card = (NetCard)selectNetCardPane.getSelectNetCard(); 
                }
                setDhcpPane.initDhcpSetupPane( ibootSrvList,subnetList,card.mac,isUseOdyDhcp,dhcpClntInfo,ip,ip_3rd,nextsrv_3rd );
            }else{
                setTipTextOnTopPane( SanBootView.res.getString("InitBootHostWizardDialog.title5") ); 
                Object val = dhcpOnClnt.get( ip );
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.title5").trim() ) == 0 ){ 
            ...
            if( ahost != null ){
                dupStrgyPane.setIPAndPort( ip,port,mtpp_port,ahost.getName(), ahost.getMachine(),ahost.getOS(), ahost.getUUID(),mac,"" );
            }else{
                dupStrgyPane.setIPAndPort( ip,port,mtpp_port,getAgentInfo.getHostName(), getAgentInfo.getMachine(), getAgentInfo.getOSName(), uuid,mac,"" );
            }
        ...
        if( getTipTitle().compareTo(SanBootView.res.getString("InitBootHostWizardDialog.title6").trim() ) == 0 ){ 
            ...
            if( !selectNetBootModePane.isSelectIScsiHBAMode() ){              
                if( selectNetCardPane.getSelectNetCard() != null){
                    ...
                    runningTaskPaneForCMDP.ctrDhcpCb( setDhcpPane.isManualSetup(),"" );
                }
            }else{
...
//        hostNode.add( chiefLProfNode );
    ...
    private int getMaxSnapSetOnGUI( BootHost host,String drive_letter ){
        ArrayList<PPProfile> curNewPPPList = (ArrayList<PPProfile>)this.ppprofOnClnt.get( host.getIP() );
        if( curNewPPPList == null) return 0;
    ...
    public void initBootHost(){
        ...
        NetCard netCard = new NetCard( "00-00-00-00-00-00" );
        Volume osVol;
...
SanBootView.log.error( getClass().getName(),"setInitType_pause: "+errMsg );
        ...
        if( ( crtSize > 0 ) || ( formatSize > 0 ) || ( selSize > 0 ) ){
            for( i=0; i<crtSize; i++ ){
                ...
                driver = binder.part.disklabel.substring(0,1).toUpperCase();
...
SanBootView.log.error( getClass().getName(),"setInitType_pause: "+errMsg );
                    ...
                    if( binder.desc.length() == 0 ){ binder.desc = "mg-"+driver; }
...
SanBootView.log.error(getClass().getName(), " target server name is null.");                
                    ...
                    // add portal
                    addPortal = view.initor.mdb.addPortal( ip, mtpp_port, view.initor.getTxIP(),ResourceCenter.ISCSI_LOGIN_PORT );
                    if( !addPortal ){
                        formatVolStepOK = false;
                        errMsg = SanBootView.res.getString("InitBootHostWizardDialog.log.addPortal") + " [ "+ view.initor.getTxIP() +","+ResourceCenter.ISCSI_LOGIN_PORT +" ] "+
                                   SanBootView.res.getString("common.failed"); 
                        runningTaskPaneForCMDP.setLogOnTabpane( errMsg, RunningTaskPaneForCMDP.CRT_VOL_ROW );
                        writeLogBuf( errMsg, RunningTaskPaneForCMDP.CRT_VOL_ROW );
                    }else{
                                    ...
                                    if( view.initor.mdb.hasThisTargetDisk( iscsiVar ) ){
                                        ...
                                        if( view.initor.mdb.loginTarget( 
                                            ...
                                            isOk = view.initor.mdb.formatVol(
                                                ...
                                                iscsiVar, 
                                                binder.part.fsType,
                                                ResourceCenter.VOL_LABEL_PREFIX+lastItemOfIP+"-"+binder.part.disklabel.substring(0,1),
...
SanBootView.log.info( getClass().getName()," @@#################  Get other info END ###################@@ ");
        ...
        //saveInfoStepOK = true;
        boolean isOkForReCrtPPP;
        boolean isOkForCrtVolMapFromCrtList;
        boolean isOkForCrtVolMapFromSelList;
        boolean isCopyOS;
        boolean isOkForSaveIp = true;
        boolean isOkForSavePartition;
        boolean isOkForSavePartition1;
...
SanBootView.log.info( getClass().getName()," @@#################  Save info Begin ###################@@ ");
        ...
        ReCrtPPProfileWhenInit reCrtPPPWheninit = new ReCrtPPProfileWhenInit( view,host,runningTaskPaneForCMDP,this,curVolMapList,volInfo,curNewPPPList,curPPPList );
...
SanBootView.log.error( getClass().getName()," Error: target server name is null.");                
                    ...
                    try {
                        ...
                        Logger.getLogger(PhyInitBootHostWizardDialog.class.getName()).log(Level.SEVERE, null, ex);
                    }
...
//        }
        ...
        if( needAddIboot && selectNetBootModePane.isSelectEmBootMode() ){
            if( hasCrtOSDisk && OSBinder != null ){ // os volume 是创建产生的
...
//                }
                ...
                view.initor.mdb.addLunMap( tid1, iboot_dhcp_ip, "255.255.255.255", "rw", "", "", "", "" );
                ...
                if( isOk ){
                    if( view.initor.mdb.getFromVectorOnMac( NetCard.getSimpleMac( netCard.mac ) ) != null ){
                        view.initor.mdb.modIboot( NetCard.getSimpleMac( netCard.mac ), IBootObj.IBOOT_PORTALIP, view.initor.getTxIP() );
...
//SanBootView.log.debug(getClass().getName(),"lv label; " + lv.getVolDiskLabel() );
    ...
    private String getDestDevPathForWin( boolean withLabel ){
        ...
        StringBuffer drivepath_sb = new StringBuffer("");
        boolean isFirst = true;
...
SanBootView.log.debug( getClass().getName()," mp size:" + winSrcList.size() );
        ...
        while( list.hasMoreElements() ){
            ...
            StringBuffer src_sb = new StringBuffer(src);
            src_sb.append(":\\");                       //src += ":\\";
            src = src_sb.toString();
...
SanBootView.log.error( getClass().getName(), "\nLV is null in MDB: " + src );
                ...
                drivepath_sb = new StringBuffer("");
                break;
            }else{
                if( isFirst ){
                    if( !withLabel ){
                        drivepath_sb = new StringBuffer(ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
                                ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+lv.getVolTargetID());
                    }else{
                        drivepath_sb = new StringBuffer(src+"-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
                            ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+lv.getVolTargetID());
                    }
                    isFirst = false;
                }else{
                    if( !withLabel ){
                        drivepath_sb.append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(lv.getVolTargetID());
//                        drivepath+="-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                            ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+lv.getVolTargetID();
                    }else{
                        drivepath_sb.append("-?").append(src).append("-?").append(ResourceCenter.TARGET_TYPE).append(view.initor.getTxIP()).append(":").append(ResourceCenter.ISCSI_LOGIN_PORT).append("@").append(ResourceCenter.ISCSI_PREFIX).append(this.targetSrvName).append(":").append(lv.getVolTargetID());
//                        drivepath+="-?"+src+"-?"+ResourceCenter.TARGET_TYPE + view.initor.getTxIP()+":"+ResourceCenter.ISCSI_LOGIN_PORT+"@"+
//                            ResourceCenter.ISCSI_PREFIX+this.targetSrvName+":"+lv.getVolTargetID();
                    }
                }
            }
        }
        drivepath = drivepath_sb.toString();
        return drivepath;
    }
...
SanBootView.log.error(getClass().getName(), "\nVG or Target is null,exit." );
    ...
    private String getMgInfoForWin(){
        ...
        MirrorGrp mg;
        int mgid;
        StringBuffer mgInfo_sb = new StringBuffer("");
...
SanBootView.log.error( getClass().getName(), "\nLV is null in MDB: " + src +":\\" );
                ...
                mgInfo_sb.append(lv.getVolTargetID()).append("-?").append(lv.getMaxSnapNum()).append("-?").append(mgid).append(";");
                //mgInfo += lv.getVolTargetID()+"-?"+lv.getMaxSnapNum()+"-?"+mgid+";";
            }
        }
        String mgInfo =  mgInfo_sb.toString();
        return mgInfo;
    }
...
SanBootView.log.error( getClass().getName()," Error: target server name is null.");
        ...
        String aWindir = view.initor.mdb.getWinDir( host.getIP(),host.getCmdpPort(),ResourceCenter.CMD_TYPE_CMDP );
        if( aWindir.length() == 0 ){
...
SanBootView.log.error( getClass().getName()," Error: getting src_dest_mapper for profile failed.");
                ...
                sb.append(" : ").append(profName);      //errMsg += " : "+profName;
                errMsg = sb.toString();
                runningTaskPaneForCMDP.setLogOnTabpane( errMsg,RunningTaskPane.SAVE_INFO_ROW );
                writeLogBuf( errMsg, RunningTaskPane.SAVE_INFO_ROW );
                return false;
...
class GetInitInfoForPhyThread extends Thread{
    ...
    private String diskDetailInfo;
    private GetAgentInfo getAgentInfo = new GetAgentInfo();
    private String uuid = "";
            ...
            try{
                ...
                if( isOk ){
                    if( !getAgentInfo.getOSName().toUpperCase().startsWith("WIN") ){
                        ...
                        if( view.initor.mdb.getUUID().length() == 0 ){
...
SanBootView.log.error( getClass().getName(),"Can't get disk partition info,so will not auto-restore-partition later. But we will tolerate this error.");
        ...
        try {
            ...
            Logger.getLogger(GetInitInfoForPhyThread.class.getName()).log(Level.SEVERE, null, ex);
