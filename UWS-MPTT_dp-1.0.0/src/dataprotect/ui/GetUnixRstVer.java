...
public class GetUnixRstVer extends Thread implements StartupProgress {
    ...
    SanBootView.log.warning(getClass().getName(), "Can't get clone disk info on [ " + hid +"/0" +"/"+vol_rootid+" ]");
                    ...
                    for( j=0; j<size1; j++ ){
                        ...
                        if( isFirst ){
                            where = new StringBuffer(BasicVDisk.BVDisk_Snap_Root_ID + "=" + cloneDisk.getRoot_id());
                            isFirst = false;
                        }else{
                            where.append(" or ").append(BasicVDisk.BVDisk_Snap_Root_ID).append("=").append(cloneDisk.getRoot_id());
                ...
                if( isFirst ){
                    where =  new StringBuffer(BasicVDisk.BVDisk_Snap_Root_ID + "=" + tgt.getVol_rootid());
                    isFirst = false;
                }else{
                    where.append(" or ").append(BasicVDisk.BVDisk_Snap_Root_ID).append("=").append(tgt.getVol_rootid());
...
SanBootView.log.error( getClass().getName(),"Not found Tgt volmap: "+ lv.getVolName() );                    
        ...
        isOk = view.initor.mdb.queryVSnapDB(
            "select * from " + ResourceCenter.VSnap_DB +" where "+ where.toString() +";"
        );
        if( isOk ){
            for( i=0; i<size; i++ ){
                lv = (VolumeMap)lvList.elementAt(i);
...
SanBootView.log.debug( getClass().getName(), " tgt id: "+ tgt.getVolTargetID()  );
        ...
        try {
            ...
            Logger.getLogger(GetUnixRstVer.class.getName()).log(Level.SEVERE, null, ex);
