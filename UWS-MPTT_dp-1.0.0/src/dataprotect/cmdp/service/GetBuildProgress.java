...
import dataprotect.data.BindOfSrcAndDest;
import java.util.ArrayList;
import java.util.HashMap;
import dataprotect.ui.RunningTask;
...
public class GetBuildProgress extends Thread {
    ...
    Runnable setVal = new Runnable(){
        public void run(){
            ...
            if( log.length() == 0 && prefix.length() == 0 ) return;
...
SanBootView.log.info( getClass().getName(), "finished task info: \n" + endTask.toString() );
                    sb.append(endTask);     //prefix += endTask.toString();
                }
            }else{
                if( isFirst ){
                    isFirst = false;
                ...
                if( record.getPercent().length() == 0 ){
                    if( olderRec == null || olderRec.getPercent().length() == 0 ){
                ...
                if( record.getRemainTime().length() == 0 ){
                    if( olderRec == null || olderRec.getPercent().length() == 0 ){
                ...
                if( record.getElapsedTime().length() == 0 ){
                    if( olderRec == null || olderRec.getPercent().length() == 0 ){
                ...
                if( record.getFinished().length() == 0 ){
                    if( olderRec == null || olderRec.getPercent().length() == 0 ){
    ...
    private void saveOlderRecord(){
        ...
        for( int i=0; i<size; i++ ){
            ...
            if( olderRec == null ){
                ...
                if( curRec.getFinished().length() != 0 ){
