...
public class SelectRestoreDiskPane extends javax.swing.JPanel {
    ...
    private void initComponents() {
        ...
        jPanel7.setMaximumSize(new java.awt.Dimension(205, 32));
    ...
    private void myInit( SanBootView _view,DestAgent _host,RestoreOriginalDiskInterface _diag,int mode ){
        ...
        jLabel2.setText( SanBootView.res.getString("SelectRestoreDiskPane.label.realRestOsLetter2") );
            ...
            if( mode == ResourceCenter.CMD_TYPE_MTPP ){
                ...
                if( host.isRealBootHost() && volMap.isCMDPProtect() ){
                    ...
                    if( prof == null ){
                        ...
                        RestoreSeq restoreSeq = seqMap.get( prof );
                        if( restoreSeq != null){
                            data[i][2] = new Integer( restoreSeq.seqVal );
                        }else
                            data[i][2] = 0 ;
                    }
                }else{
    ...
    public boolean hasRestoredDisk(){
        ...
        int lineNum = model.getRowCount();
        for( int row=0; row<lineNum; row++ ){
            Boolean isSel = (Boolean)model.getValueAt(row, 4 );
            if( isSel.booleanValue() ){
                cnt++;
            }
        }
        ...
        for( int row=0; row<lineNum; row++ ){
            ...
            if( !matcher.find() ){
                JOptionPane.showMessageDialog(this,
                    SanBootView.res.getString("SelectRestoreDiskPane.error.invalidVal") + wraper.volMap.getVolDiskLabel()
                );
                return false;
            }
            ...
            if( !badDestDriver( destStr ) ){
                JOptionPane.showMessageDialog(this,
                    SanBootView.res.getString("SelectRestoreDiskPane.error.rstOntoSelf") + wraper.volMap.getVolDiskLabel()
                );
                return false;
            }
            ...
            if( isOk ){
                if( view.initor.mdb.isStartupFromSAN() ){
                    JOptionPane.showMessageDialog(this,
                        SanBootView.res.getString("SelectRestoreDiskPane.error.notLocalDisk") + val.toString()
                    );
                    return false;
                }
            }else{
                JOptionPane.showMessageDialog(this,
                    SanBootView.res.getString("SelectRestoreDiskPane.error.getStartupInfo") +  val.toString()
                );
                return false;
            }
                ...
                if( mode ==  ResourceCenter.CMD_TYPE_CMDP ){
                    ...
                    if( !retVal.isOk ){
                        ...
                        ret = JOptionPane.showConfirmDialog(
                            view,
                            SanBootView.res.getString("MenuAndBtnCenter.confirm31")  + destStr + SanBootView.res.getString("MenuAndBtnCenter.confirm31"),
                            SanBootView.res.getString("common.confirm"),  //"Confirm",
                            JOptionPane.OK_CANCEL_OPTION
                        );
                        if(  ( ret == JOptionPane.CANCEL_OPTION ) || (  ret == JOptionPane.CLOSED_OPTION) ){
                        ...
                        boolean isequ = view.initor.mdb.expandTargetPartition(ip, port, destStr);
                        
                        if ( isequ ){
                            if ( host.isWin2000() ){
                                JOptionPane.showMessageDialog(this,
                                    SanBootView.res.getString("SelectRestoreDiskPane.common.2000reboot")
                                );
                                return false;
                            } else {
                                ret = JOptionPane.showConfirmDialog(
                                    view,
                                    SanBootView.res.getString("SelectRestoreDiskPane.common.otherSystem"),
                                    SanBootView.res.getString("common.confirm"),  //"Confirm",
                                    JOptionPane.OK_CANCEL_OPTION
                                );
                        if(  ( ret == JOptionPane.CANCEL_OPTION ) || (  ret == JOptionPane.CLOSED_OPTION) ){
                            ...
                            JOptionPane.showMessageDialog(this,
                                SanBootView.res.getString("SelectRestoreDiskPane.error.expandpartitionfailed") + destStr
                            );
                            return false;
                        }
    ...
    public void setJTextField1(String driverletter){
        this.jTextField1.setText(driverletter);
    }
}
