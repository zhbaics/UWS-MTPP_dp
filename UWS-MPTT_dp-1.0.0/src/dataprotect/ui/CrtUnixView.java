...
public class CrtUnixView extends Thread{
    ...
    StringBuffer selVer_buf;  // save selected version info
            ...
            if( useOdyDhcp ){
                ...
                if( isOk ){                    
                    try {
                        ...
                        Logger.getLogger(CrtUnixView.class.getName()).log(Level.SEVERE, null, ex);
                    }
...
//System.out.println(" tgt name: "+tgt.getVolName() +" view id: "+tgt.getVol_view_targetid() );
            ...
            if( isFirst ){
                tmp = new StringBuffer(tid + "");
                isFirst = false;
            }else{
                tmp.append(";").append(tid);        //tmp += ";" + tid;
            }
        }
        buf.append("\ntargetlist="+tmp+"\n");
        ...
        for( i=0; i<size; i++ ){
            ...
            if( tmp.length() != 0 ){
                ...
                for( j=0; j<size1; j++ ){
                    ...
                    if( isFirst ){
                        tmp = new StringBuffer(lv.getVolName());
                        isFirst = false;
                    }else{
                        tmp.append(";").append(lv.getVolName());        //tmp+=";"+lv.getVolName();
                    }
                }
                buf.append("\nlvs="+tmp);
