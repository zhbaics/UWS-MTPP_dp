...
import dataprotect.remotemirror.ui.multiRenderTable.JTableForMjSch;
import dataprotect.remotemirror.ui.multiRenderTable.MyDefaultTableModelForMJSch;
...
public class EditMjSchDialog extends javax.swing.JDialog {
    ...
    */
    ...
    private void setupTable(){
        ...
        for( int i=0; i<24; i++ ){
            data[i][0] = i+"";
            data[i][1] = Boolean.valueOf( true );
            data[i][2] = Boolean.valueOf( true );
            data[i][3] = Boolean.valueOf( true );
            data[i][4] = Boolean.valueOf( true );
            data[i][5] = Boolean.valueOf( true );
            data[i][6] = Boolean.valueOf( true );
            data[i][7] = Boolean.valueOf( true );
        }
    ...
    private void editTable( String[] selTime,int col ){
       ...
       for( int i=0; i<24; i++ ){
           model.setValueAt( Boolean.valueOf( this.isContainedInSelectedTime( selTime, i+"" ) ), i, col );
       }
    }
    ...
    private void do_defaultTimeBox(){
        ...
        for( int row=0; row<lineNum; row++ ){
            ...
            if( !this.isContainedInDefaultTime( clock ) ) {
                for( int j=1; j<8; j++ ){
                    model.setValueAt( Boolean.valueOf( false ), row, j );
                }
            }else{
                for( int j=1; j<8; j++ ){
                    model.setValueAt( Boolean.valueOf( this.jCheckBox1.isSelected() ), row, j );
                }
            }
            fireEditingStopMsg();
        }
    ...
    private void do_selAllBox(){
        ...
        for( int row=0; row<lineNum; row++ ){
            for( int j=1; j<8; j++ ){
                model.setValueAt( Boolean.valueOf( this.jCheckBox2.isSelected() ), row, j );
            }
            fireEditingStopMsg();
        }
        fireEditingStopMsg();
